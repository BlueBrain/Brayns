...
// Adapted to the ASSIMP library because the builtin atof indeed takes AGES to
// parse a
// float inside a large string. Before parsing, it does a strlen on the given
// point.
// Changes:
//  22nd October 08 (Aramis_acg): Added temporary cast to double, added
//  strtoul10_64
...
namespace Assimp
{
const double fast_atof_table[16] = { // we write [16] here instead of [] to work
                                     // around a swig bug
    0.0,
    0.1,
    0.01,
                                     ...
                                     bool check_comma = true)
{
        ...
        c[1] <= '9')
    {
        ...
        // NOTE: The original implementation is highly inaccurate here. The
        // precision of a single
        // IEEE 754 float is not high enough, everything behind the 6th digit
        // tends to be more
        // inaccurate than it would need to be. Casting to double seems to solve
        // the problem.
        // strtol_64 is used to prevent integer overflow.

        // Another fix: this tends to become 0 for long numbers if we don't
        // limit the maximum
        // number of digits to be read. AI_FAST_ATOF_RELAVANT_DECIMALS can be a
        // value between
        // 1 and 15.
    ...
    // A major 'E' must be allowed. Necessary for proper reading of some DXF
    // files.
    ...
    if (*c == 'e' || *c == 'E')
    {
        ...
        // The reason float constants are used here is that we've seen cases
        // where compilers
        // would perform such casts on compile-time constants at runtime, which
        // would be
        // bad considering how frequently fast_atoreal_move<float> is called in
        // Assimp.
