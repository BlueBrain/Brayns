...
void Scene::loadMeshFolder()
{
    ...
    if(boost::filesystem::exists(folder) && boost::filesystem::is_directory(folder))
    {
        for(boost::filesystem::directory_iterator dirIter(folder); dirIter!=endIter; ++dirIter)
        {
            ...
            {
                boost::filesystem::path fileExtension = dirIter->path().extension();
                if(fileExtension==".obj" || fileExtension==".ply" || fileExtension==".dae" ||
                   fileExtension==".3ds")
                {
                    const std::string& filename = dirIter->path().string();
                    BRAYNS_INFO << "- " << filename << std::endl;
...
void Scene::buildEnvironment()
{
    ...
    {
        // Ground
        const Vector3f scale = Vector3f(4.f,0.5f,4.f);
        const float tiles = 8.f;
        const float S = 0.5f*std::min(_bounds.getDimension().x(), _bounds.getDimension().z());
        ...
        Vector4f v;
        const Vector4f n = {0,1,0};
        v = Vector4f(c.x()-s.x()*scale.x(), c.y()-s.y()*scale.y()*1.001f, c.z()-s.z()*scale.z(), 0.f);
        _trianglesMeshes[material].getVertices().push_back(v);
        _trianglesMeshes[material].getNormals().push_back(n);
        _trianglesMeshes[material].getTextureCoordinates().push_back(Vector2f(0.f,0.f));
        v = Vector4f( c.x()+s.x()*scale.x(), c.y()-s.y()*scale.y()*1.001f, c.z()-s.z()*scale.z());
        _trianglesMeshes[material].getVertices().push_back(v);
        _trianglesMeshes[material].getNormals().push_back(n);
        _trianglesMeshes[material].getTextureCoordinates().push_back(Vector2f(tiles,0.f));
        v = Vector4f( c.x()+s.x()*scale.x(), c.y()-s.y()*scale.y()*1.001f, c.z()+s.z()*scale.z());
        _trianglesMeshes[material].getVertices().push_back(v);
        _trianglesMeshes[material].getNormals().push_back(n);
        _trianglesMeshes[material].getTextureCoordinates().push_back(Vector2f(tiles,tiles));
        ...
        _trianglesMeshes[material].getVertices().push_back(v);
        _trianglesMeshes[material].getNormals().push_back(n);
        _trianglesMeshes[material].getTextureCoordinates().push_back(Vector2f(0.f,tiles));
        v = Vector4f( c.x()-s.x()*scale.x(), c.y()-s.y()*scale.y()*1.001f, c.z()-s.z()*scale.z());
        _trianglesMeshes[material].getVertices().push_back(v);
        _trianglesMeshes[material].getNormals().push_back(n);
        _trianglesMeshes[material].getTextureCoordinates().push_back(Vector2f(0.f,0.f));
    ...
    case SE_WALL:
    {
        ...
        size_t meshIndex = _trianglesMeshes[material].getIndices().size();
        Vector4f v;
        const Vector4f n = {0,0,-1};
        v = Vector4f( c.x()-s.x()*scale.x(), c.y()-s.y()*scale.y(), c.z()+s.z()*scale.z()*1.001f);
        _trianglesMeshes[material].getVertices().push_back(v);
        _trianglesMeshes[material].getNormals().push_back(n);
        _trianglesMeshes[material].getTextureCoordinates().push_back(Vector2f(0.f,0.f));
        v = Vector4f( c.x()+s.x()*scale.x(), c.y()-s.y()*scale.y(), c.z()+s.z()*scale.z()*1.001f);
