...
public:
    explicit EnumHandler(Enum* value)
        : m_value(value)
    {
    }
    ...
    bool String(const char* str, SizeType sz, bool) override
    {
        ...
        {
            if (pair.first.size() == sz &&
                memcmp(pair.first.data(), str, sz) == 0)
            {
                *m_value = pair.second;
                this->parsed = true;
                return true;
        ...
        the_error.reset(
            new error::InvalidEnumError(std::string(str, str + sz)));
        return false;
    }
    ...
    bool write(IHandler* output) const override
    {
        ...
        for (const std::pair<std::string, Enum>& pair : mapping)
        {
            ...
            {
                output->String(pair.first.data(),
                               static_cast<SizeType>(pair.first.size()), false);
                return true;
            }
        }
        return false;
                         ...
                         MemoryPoolAllocator& alloc) const override
    {
        ...
        for (const std::pair<std::string, Enum>& pair : mapping)
        {
            enumerations.PushBack(rapidjson::StringRef(pair.first.data(),
                                                       pair.first.size()),
                                  alloc);
        }
...
#define STATICJSON_DECLARE_ENUM(type, ...)                                    \
    ...
    {                                                                         \
    public:                                                                   \
        explicit Handler(type* value)                                         \
            : EnumHandler<type, Handler<type>>(value)                         \
        {                                                                     \
        }                                                                     \
        std::string type_name() const override { return #type; }              \
        static const std::vector<std::pair<std::string, type>>& get_mapping() \
        {                                                                     \
            static std::vector<std::pair<std::string, type>> mapping{         \
                __VA_ARGS__};                                                 \
            return mapping;                                                   \
        }                                                                     \
    };                                                                        \
