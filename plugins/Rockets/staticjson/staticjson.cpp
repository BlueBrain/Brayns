...
std::string error::Success::description() const
{
    return "No error";
}
...
{
    return "Type mismatch between expected type " + quote(expected_type()) +
           " and actual type " + quote(actual_type());
}
...
std::string error::CorruptedDOMError::description() const
{
    return "JSON has invalid structure";
}
...
std::string error::CustomError::description() const
{
    return m_message;
}
...
std::string ParseStatus::description() const
{
    ...
    if (has_error())
    {
        res = stringprintf(
            ...
            static_cast<long long>(m_offset), m_code,
            rapidjson::GetParseError_En(
                static_cast<rapidjson::ParseErrorCode>(m_code)));
        if (m_stack)
        {
...
bool IHandler::RawNumber(const char*, SizeType, bool)
{
    fprintf(
        stderr, "%s",
        "Calling non-overridden IHandler::RawNumber() is a programming bug!\n");
    std::terminate();
}
...
std::string ObjectHandler::type_name() const
{
    return "object";
}
...
void ObjectHandler::set_missing_required(const std::string& name)
{
    ...
    std::vector<std::string>& missing =
        static_cast<error::RequiredFieldMissingError*>(the_error.get())
            ->missing_members();
...
bool ObjectHandler::write(IHandler* output) const
{
    ...
    for (auto&& pair : internals)
    {
        ...
        if (!output->Key(pair.first.data(),
                         static_cast<staticjson::SizeType>(pair.first.size()),
                         true))
            return false;
...
void ObjectHandler::generate_schema(Value& output,
                                    MemoryPoolAllocator& alloc) const
{
    output.SetObject();
    ...
    output.AddMember(rapidjson::StringRef("additionalProperties"),
                     !(get_flags() & Flags::DisallowUnknownKey), alloc);
}
...
bool parse_json_string(const char* str, BaseHandler* handler,
                       ParseStatus* status)
{
...
bool read_value(Value* v, MemoryPoolAllocator* alloc, const BaseHandler* input,
                ParseStatus* status)
{
...
std::string JSONHandler::type_name() const
{
    return "JSON";
}
...
bool JSONHandler::write(IHandler* output) const
{
    return m_value->Accept(*output);
}
}
