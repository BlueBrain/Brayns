...
protected:
    ...
    {
        the_error.reset(new error::ArrayElementError(m_value->size()));
    }
...
public:
    ...
    bool String(const char* str, SizeType length, bool copy) override
    {
        return precheck("string") &&
               postcheck(internal.String(str, length, copy));
    }
    ...
    bool write(IHandler* output) const override
    {
        ...
        return output->EndArray(
            static_cast<staticjson::SizeType>(m_value->size()));
    }

    void generate_schema(Value& output,
                         MemoryPoolAllocator& alloc) const override
    {
        output.SetObject();
        output.AddMember(rapidjson::StringRef("type"),
                         rapidjson::StringRef("array"), alloc);
        Value items;
...
{
public:
    explicit Handler(std::deque<T>* value)
        : ArrayHandler<std::deque<T>>(value)
    {
    }
...
{
public:
    explicit Handler(std::list<T>* value)
        : ArrayHandler<std::list<T>>(value)
    {
    }
...
protected:
    void set_element_error()
    {
        the_error.reset(new error::ArrayElementError(count));
    }

    void set_length_error()
    {
        the_error.reset(new error::ArrayLengthMismatchError());
    }
...
protected:
    explicit PointerHandler(PointerType* value)
        : m_value(value)
    {
    }
...
public:
                         ...
                         MemoryPoolAllocator& alloc) const override
    {
        ...
        nullDescriptor.AddMember(rapidjson::StringRef("type"),
    ...
    {
        return internal_handler && internal_handler->has_error();
    }
...
class Handler<std::unique_ptr<T, Deleter>>
    : public PointerHandler<std::unique_ptr<T, Deleter>>
{
public:
...
{
public:
    explicit Handler(std::shared_ptr<T>* value)
        : PointerHandler<std::shared_ptr<T>>(value)
    {
    }
...
public:
    ...
    bool EndArray(SizeType length) override
    {
        return precheck("array") &&
               postcheck(internal_handler.EndArray(length));
    }
    ...
    bool write(IHandler* out) const override
    {
        ...
        {
            if (!out->Key(pair.first.data(),
                          static_cast<SizeType>(pair.first.size()), true))
                return false;
...
public:
    explicit Handler(
        std::unordered_multimap<std::string, T, Hash, Equal>* value)
...
public:
    ...
    std::string type_name() const override
    {
        return "std::multimap<std::string, " +
               this->internal_handler.type_name() + ">";
    }
};
...
struct TupleIniter
{
    void operator()(std::unique_ptr<BaseHandler>* handlers, Tuple& t) const
    {
        handlers[index].reset(
            new Handler<typename std::tuple_element<index, Tuple>::type>(
                &std::get<index>(t)));
...
class Handler<std::tuple<Ts...>>
    : public TupleHander<std::tuple_size<std::tuple<Ts...>>::value>
{
private:
