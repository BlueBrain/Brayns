...
protected:
    ...
    {
        typedef
            typename std::common_type<IntType, AnotherIntType>::type CommonType;
        typedef typename std::numeric_limits<IntType> this_limits;
        typedef typename std::numeric_limits<AnotherIntType> that_limits;

        // The extra logic related to this_limits::min/max allows the compiler
        // to
...
public:
    explicit IntegerHandler(IntType* value)
        : m_value(value)
    {
    }
                         ...
                         MemoryPoolAllocator& alloc) const override
    {
        ...
        output.AddMember(rapidjson::StringRef("type"),
                         rapidjson::StringRef("integer"), alloc);
...
{
public:
    explicit Handler(long* i)
        : IntegerHandler<long>(i)
    {
    }
...
{
public:
    explicit Handler(unsigned long* i)
        : IntegerHandler<unsigned long>(i)
    {
    }
...
{
public:
    explicit Handler(long long* i)
        : IntegerHandler<long long>(i)
    {
    }
...
{
public:
    explicit Handler(unsigned long long* i)
        : IntegerHandler<unsigned long long>(i)
    {
    }
