...
public:
    ...
    bool EndObject(SizeType memberCount)
    {
        ...
        for (SizeType i = 0; i < memberCount; i++)
            h ^= Hash(kv[i * 2], kv[i * 2 + 1]); // Use xor to achieve member
                                                 // order insensitive
...
public:
        ...
        , defaultValueLength_(0)
    {
        ...
        {
            schemaDocument->CreateSchema(&not_,
                                         p.Append(GetNotString(), allocator_),
                                         *v, document);
...
#define RAPIDJSON_STRING_(name, ...)                                  \
    ...
    {                                                                 \
        ...
        static const ValueType v(s, static_cast<SizeType>(            \
                                        sizeof(s) / sizeof(Ch) - 1)); \
        return v;                                                     \
    }
...
/*!
   ...
   construction).
    \tparam ValueT Type of JSON value (e.g. \c Value ), which also determine the
   encoding.
...
public:
    ...
    /*!
       ...
       resolving remote reference. Can be null.
        \param allocator An optional allocator instance for allocating memory.
       Can be null.
    */
...
private:
    ...
    internal::Stack<Allocator> schemaRef_; // Stores Pointer from $ref and
                                           // schema which holds the $ref
    URIType uri_;
};
...
public:
    ...
    /*!
       ...
       states.
        \param schemaStackCapacity Optional initial capacity of schema path
       stack.
        \param documentStackCapacity Optional initial capacity of document path
       stack.
    */
    GenericSchemaValidator(
...
#define RAPIDJSON_STRING_(name, ...)                                      \
    ...
    {                                                                     \
        static const Ch s[] = {__VA_ARGS__, '\0'};                        \
        static const StringRefType v(s, static_cast<SizeType>(            \
                                            sizeof(s) / sizeof(Ch) - 1)); \
        return v;                                                         \
    }
...
private:
    ...
    {
        documentStack_.template Reserve<Ch>(1 + len * 2); // worst case all
                                                          // characters are
                                                          // escaped as two
                                                          // characters
                       ...
                       ISchemaValidator** subvalidators, SizeType count)
    {
        ValueType errors(kArrayType);
        for (SizeType i = 0; i < count; ++i)
            errors.PushBack(static_cast<GenericSchemaValidator*>(
                                subvalidators[i])
                                ->GetError(),
                            GetStateAllocator());
        currentError_.SetObject();
    ...
    internal::Stack<StateAllocator> schemaStack_;   //!< stack to store the
                                                    //!current path of schema
                                                    //!(BaseSchemaType *)
