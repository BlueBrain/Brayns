...
{
    GenericValue<Encoding, Allocator> name;  //!< name of member (must be a
                                             //!string)
...
public:
    ...
    typedef GenericStringRef<Ch> StringRefType; //!< Reference to a constant
                                                //!string
    ...
    typedef typename GenericMemberIterator<true, Encoding, Allocator>::Iterator
        ConstMemberIterator; //!< Constant member iterator for iterating in
                             //!object.
    typedef GenericValue* ValueIterator; //!< Value iterator for iterating in
                                         //!array.
    typedef const GenericValue* ConstValueIterator; //!< Constant value iterator
                                                    //!for iterating in array.
    typedef GenericValue<Encoding, Allocator> ValueType; //!< Value type of
                                                         //!itself.
 ...
 */
    ...
    */
    ...
    /*! \param str Constant string reference to be assigned
       ...
       primitive type assignment overload below.
    ...
    */
    ...
    /*! \pre IsObject() == true
       ...
       a null value. This makes issue 7.
        Since 0.2, if the name is not correct, it will assert.
        If user is unsure whether a member exists, user should use HasMember()
       first.
    ...
    GenericValue& MemberReserve(SizeType newCapacity, Allocator& allocator)
    {
        ...
        if (newCapacity > data_.o.capacity)
        {
            SetMembersPointer(reinterpret_cast<Member*>(
                allocator.Realloc(GetMembersPointer(),
    ...
    */
    ...
    /*! This function adopts the GoF visitor pattern.
       ...
       which is a Handler.
        It can also be used to deep clone this value via GenericDocument, which
       is also a Handler.
    ...
    */
    ...
    bool Accept(Handler& handler) const
    {
        ...
        case kObjectType:
            ...
            {
                RAPIDJSON_ASSERT(m->name.IsString()); // User may change the
                                                      // type of name by
                                                      // MemberIterator.
                if (RAPIDJSON_UNLIKELY(
                        !handler.Key(m->name.GetString(),
...
private:
    ...
    struct Flag
    {
        ...
        char payload[sizeof(SizeType) * 2 + sizeof(void*) + 6]; // 6 padding
                                                                // bytes
#else
    ...
    // up to MaxSize chars
    // (excluding the terminating zero) and store a value to determine the
    // length of the contained
    // string in the last character str[LenPos] by storing "MaxSize - length"
    // there. If the string
    // to store has the maximal length of MaxSize then str[LenPos] will be 0 and
    // therefore act as
    // the string terminator as well. For getting the string length back from
    // that value just use
    // "MaxSize - str[LenPos]".
    // This allows to store 13-chars strings in 32-bit mode, 21-chars strings in
    // 64-bit mode,
    // 13-chars strings for RAPIDJSON_48BITPOINTER_OPTIMIZATION=1 inline (for
...
public:
    typedef typename Encoding::Ch Ch; //!< Character type derived from Encoding.
    typedef GenericValue<Encoding, Allocator> ValueType; //!< Value type of the
                                                         //!document.
    typedef Allocator AllocatorType; //!< Allocator type from template
                                     //!parameter.
    ...
    */
    ...
    GenericDocument(GenericDocument&& rhs) RAPIDJSON_NOEXCEPT
        : ValueType(std::forward<ValueType>(rhs)), // explicit cast to avoid
                                                   // prohibited move from
                                                   // Document
...
private:
    ...
    {
        if (Allocator::kNeedFree)
            while (stack_.GetSize() > 0) // Here assumes all elements in stack
                                         // array are GenericValue (Member is
