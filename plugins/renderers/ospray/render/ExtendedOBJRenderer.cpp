...
void ExtendedOBJRenderer::commit()
{
    ...
    if (lightData)
        for (size_t i=0; i < lightData->size(); ++i)
            lightArray.push_back(((ospray::Light**)lightData->data)[i]->getIE());
    ...
    frameNumber = getParam1i("frameNumber",0);
    spp = getParam1i("spp", 1);
    electronShadingEnabled = bool(getParam1i("electronShading", 0));
    lightEmittingMaterialsEnabled = bool(getParam1i("lightEmittingMaterialsEnabled", 0));
    gradientBackgroundEnabled = bool(getParam1i("gradientBackgroundEnabled", 0));
    ...
    if (materialData)
        ...
        {
            materialArray.push_back(((ospray::Material**)materialData->data)[i]->getIE());
        }
