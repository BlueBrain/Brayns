...
OSPRayRenderer::OSPRayRenderer(
        ...
        RenderingParametersPtr renderingParamaters )
    : Renderer( argc, argv, renderingParamaters)
{
    ospInit(&argc,argv);
...
{
    OSPRayFrameBuffer* osprayFrameBuffer = dynamic_cast<OSPRayFrameBuffer*>(frameBuffer.get());
    ospRenderFrame(osprayFrameBuffer->impl(), _renderer, OSP_FB_COLOR|OSP_FB_ACCUM);
}
...
void OSPRayRenderer::commit()
{
    ...
    ospSet1i(_renderer, "softShadowsEnabled", _renderingParameters->getSoftShadows());
    ospSet1f(_renderer, "ambientOcclusionStrength",
        _renderingParameters->getAmbientOcclusionStrength());
    ospSet1i(_renderer, "shadingEnabled", _renderingParameters->getLightShading());
    ...
    ospSet1i(_renderer, "electronShading", _renderingParameters->getElectronShading());
...
void OSPRayRenderer::setCamera( CameraPtr camera )
{
    ...
    assert(osprayCamera);
    ospSetObject(_renderer, "camera", osprayCamera->impl());
}
