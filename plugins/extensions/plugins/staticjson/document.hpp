...
public:
    ...
    void generate_schema(Value& output, MemoryPoolAllocator&) const override
    {
        output.SetObject();
    }
};
...
{
public:
    explicit Handler(Document* h)
        : JSONHandler(h, &h->GetAllocator())
    {
    }
};
...
template <class T>
bool from_json_document(const Document& d, T* t,
...
template <class T>
bool to_json_value(Value* v, MemoryPoolAllocator* alloc, const T& t,
                   ParseStatus* status)
{
