...
public:
    ...
    virtual void generate_schema(Value& output,
                                 MemoryPoolAllocator& alloc) const = 0;
};

struct Flags
{
    static const unsigned Default = 0x0, AllowDuplicateKey = 0x1,
                          Optional = 0x2, IgnoreRead = 0x4, IgnoreWrite = 0x8,
                          DisallowUnknownKey = 0x16;
};
...
struct Converter
{
    ...
    static std::unique_ptr<ErrorBase> from_shadow(const shadow_type& shadow,
                                                  T& value)
    {
        (void)shadow;
        (void)value;
...
public:
    explicit ConversionHandler(T* t)
        : shadow()
        , internal(&shadow)
        , m_value(t)
    {
    ...
    {
        return postprocess(internal.Uint(u));
    }

    virtual bool Int64(std::int64_t i) override
    {
        return postprocess(internal.Int64(i));
    }

    virtual bool Uint64(std::uint64_t u) override
    {
    ...
    {
        return postprocess(internal.Double(d));
    }
    ...
    virtual bool StartObject() override
    {
        return postprocess(internal.StartObject());
    }
    ...
    virtual bool EndObject(SizeType sz) override
    {
    ...
    virtual bool StartArray() override
    {
        return postprocess(internal.StartArray());
    }

    virtual bool EndArray(SizeType sz) override
    {
...
public:
    explicit DispatchHandler(T* t)
        : ::staticjson::ObjectTypeHandler<T>(t)
    {
    }
};
...
public:
    explicit DispatchHandler(T* t)
        : ::staticjson::ConversionHandler<T>(t)
    {
    }
};
}
