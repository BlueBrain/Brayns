...
void CircuitExplorerMaterial::commit()
{
    ...
    shadingMode = static_cast<MaterialShadingMode>(
        getParam1i(MATERIAL_PROPERTY_SHADING_MODE, static_cast<int>(MaterialShadingMode::none)));
    // Clipping mode
    clippingMode = static_cast<MaterialClippingMode>(
        getParam1i(MATERIAL_PROPERTY_CLIPPING_MODE, static_cast<int>(MaterialClippingMode::no_clipping)));
    ...
    ispc::CircuitExplorerMaterial_set(
        ...
        (const ispc::AffineSpace2f &)xform_Refraction,
        refraction,
        map_Reflection ? map_Reflection->getIE() : nullptr,
        (const ispc::AffineSpace2f &)xform_Reflection,
        reflection,
        map_a ? map_a->getIE() : nullptr,
        (const ispc::AffineSpace2f &)xform_a,
        a,
        glossiness,
        castSimulationData,
...
OSP_REGISTER_MATERIAL(circuit_explorer_advanced, CircuitExplorerMaterial, default);
OSP_REGISTER_MATERIAL(circuit_explorer_voxelized_simulation, CircuitExplorerMaterial, default);
OSP_REGISTER_MATERIAL(circuit_explorer_cell_growth, CircuitExplorerMaterial, default);
OSP_REGISTER_MATERIAL(circuit_explorer_proximity_detection, CircuitExplorerMaterial, default);
} // namespace obj
} // namespace brayns
