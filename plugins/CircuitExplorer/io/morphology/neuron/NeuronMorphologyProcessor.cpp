...
struct RadiusSmoother
{
    static void apply(NeuronMorphology &morphology)
    {
        ...
        {
            auto &soma = morphology.soma();
            for (auto s : soma.childrenIndices)
            {
...
void NeuronMorphologyProcessor::processMorphology(NeuronMorphology &morphology, const float radiusMultiplier)
{
    RadiusMultiplier::apply(morphology, radiusMultiplier);
    RadiusSmoother::apply(morphology);
}

NeuronMorphology NeuronMorphologyProcessor::processMorphology(const NeuronMorphology &morphology, const float multiplier)
{
    auto copy = morphology;
    processMorphology(copy, multiplier);
    return copy;
}
