...
#include <plugin/network/adapters/FrameExportdapter.h>

class ExportFramesToDiskEntrypoint
    : public brayns::Entrypoint<FrameExportManager::ExportInfo,
                                brayns::EmptyMessage>
{
public:
    ...
    virtual void onRequest(const Request& request) override
    {
        ...
        try
        {
            FrameExportManager::startNewExport(getApi(), std::move(params));
        }
        catch (const FrameExportParameterException& fpe)
        {
            throw brayns::EntrypointException(1, fpe.what());
        }
        catch (const FrameExportInProgressException&)
        {
            throw brayns::EntrypointException(
                2, "Frame export already in progress");
        }
