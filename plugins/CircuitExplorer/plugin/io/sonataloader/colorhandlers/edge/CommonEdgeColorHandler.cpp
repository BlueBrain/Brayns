    ...
    std::vector<ElementMaterialMap::Ptr> &&elements)
{
    ...
    std::sort(
        tempBuf.begin(),
        tempBuf.end(),
        [](const SynapseMaterialInfo &a, const SynapseMaterialInfo &b) { return a.id < b.id; });
    ...
    const auto nodeIds =
        _afferent ? SonataSynapses::getSourceNodes(edges, selection) : SonataSynapses::getTargetNodes(edges, selection);
...
std::vector<std::string> CommonEdgeColorHandler::_getMethodVariablesImpl(const std::string &method) const
{
...
void CommonEdgeColorHandler::_updateColorByIdImpl(const std::map<uint64_t, brayns::Vector4f> &colorMap)
{
    (void)colorMap;
