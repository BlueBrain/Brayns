...
ElementMaterialMap::Ptr SDFNeuronInstance::addToModel(brayns::Model &model) const
{
    ...
    for (const auto &entry : _sdfData->sectionTypeMap)
    {
        ...
        {
            localToGlobalIndex[geomIdx] = model.addSDFGeometry(materialId, _sdfGeometries[geomIdx], {});
        }
    }
...
MorphologyInstance::SegmentPoints SDFNeuronInstance::getSegment(const int32_t section, const uint32_t segment) const
{
    auto it = _sdfData->sectionGeometries.find(section);
    if (it == _sdfData->sectionGeometries.end())
        throw std::invalid_argument("Section " + std::to_string(section) + " not found");
...
uint64_t SDFNeuronInstance::getSegmentSimulationOffset(const int32_t section, const uint32_t segment) const
{
    auto it = _sdfData->sectionGeometries.find(section);
    if (it == _sdfData->sectionGeometries.end())
        throw std::invalid_argument("Section " + std::to_string(section) + " not found");
