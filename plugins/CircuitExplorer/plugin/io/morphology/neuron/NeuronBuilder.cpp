...
{
    const auto begin = _builders.begin();
    const auto end = _builders.end();
    const auto it = std::find_if(begin, end, [&](const std::unique_ptr<NeuronBuilder> &builder)
    {
...
std::vector<std::string> NeuronBuilderTable::getAvailableBuilderNames() const noexcept
{
    ...
    {
        result.push_back(builder->getName());
    }
    return result;
}
