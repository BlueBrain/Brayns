...
Data createData1D(const T *ptr, std::size_t itemCount, std::ptrdiff_t stride = 0)
{
    return createData(ptr, dataTypeOf<T>, {itemCount, 1, 1}, {stride, 0, 0});
}
...
struct ObjectParamReflector<std::vector<T>>
{
    static void set(OSPObject handle, const char *id, const std::vector<T> &items)
    {
        ...
        auto data = createData1D(items.data(), items.size());
