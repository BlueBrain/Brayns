...
template<typename T>
struct JsonSchemaStorage
{
    static inline const JsonSchema schema = JsonReflector<T>::getSchema();
};

template<typename T>
const JsonSchema &getJsonSchema()
{
    return JsonSchemaStorage<T>::schema;
}
...
template<typename T>
T deserialize(const JsonValue &json)
{
