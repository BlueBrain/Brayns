...
class TestRotation(unittest.TestCase):
    ...
    def test_from_quaternion(self) -> None:
        ...
        test = Rotation.from_quaternion(quaternion)
        self.assertEqual(test.quaternion, quaternion.normalized)

    def test_from_euler(self) -> None:
        euler = Vector3(34, -22, -80)
        test = Rotation.from_euler(euler, degrees=True)
    ...
    def test_from_axis_angle(self) -> None:
        axis = Vector3(1, 2, 3)
        angle = 30
        test = Rotation.from_axis_angle(axis, angle, degrees=True)
        quaternion = test.quaternion
        self.assertAlmostEqual(quaternion.norm, 1)
    ...
    def test_deserialize(self) -> None:
        test = Rotation.deserialize([0, 0, 0, 1])
        quaternion = Quaternion(0, 0, 0, 1)
        ref = Rotation.from_quaternion(quaternion)
        self.assertEqual(test, ref)

    def test_identity(self) -> None:
        quaternion = Quaternion.identity
        self.assertEqual(Rotation.identity, Rotation(quaternion))
    ...
    def test_euler_radians(self) -> None:
        ref = Vector3(34, -22, -80)
        rotation = Rotation.from_euler(ref, degrees=True)
    ...
    def test_combine(self) -> None:
        r1 = Rotation.from_axis_angle(Vector3.up, 30, degrees=True)
        r2 = Rotation.from_axis_angle(Vector3.up, 30, degrees=True)
        ref = r1.quaternion * r2.quaternion
        test = r1.combine(r2).quaternion
        self.assertEqual(test, ref)

    def test_apply(self) -> None:
        rotation = Rotation.from_euler(Vector3(22, 35, 68), degrees=True)
        value = Vector3(1, 2, 3)
