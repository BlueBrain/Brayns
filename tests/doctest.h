...
DOCTEST_MSVC_SUPPRESS_WARNING(4996) // The compiler encountered a deprecated
                                    // declaration
DOCTEST_MSVC_SUPPRESS_WARNING(4706) // assignment within conditional expression
DOCTEST_MSVC_SUPPRESS_WARNING(4512) // 'class' : assignment operator could not
                                    // be generated
DOCTEST_MSVC_SUPPRESS_WARNING(4127) // conditional expression is constant
DOCTEST_MSVC_SUPPRESS_WARNING(4820) // padding
DOCTEST_MSVC_SUPPRESS_WARNING(4625) // copy constructor was implicitly defined
                                    // as deleted
DOCTEST_MSVC_SUPPRESS_WARNING(4626) // assignment operator was implicitly
                                    // defined as deleted
DOCTEST_MSVC_SUPPRESS_WARNING(5027) // move assignment operator was implicitly
                                    // defined as deleted
DOCTEST_MSVC_SUPPRESS_WARNING(5026) // move constructor was implicitly defined
                                    // as deleted
DOCTEST_MSVC_SUPPRESS_WARNING(4623) // default constructor was implicitly
                                    // defined as deleted
DOCTEST_MSVC_SUPPRESS_WARNING(4640) // construction of local static object is
                                    // not thread-safe
// static analysis
DOCTEST_MSVC_SUPPRESS_WARNING(26439) // This kind of function may not throw.
                                     // Declare it 'noexcept'
DOCTEST_MSVC_SUPPRESS_WARNING(26495) // Always initialize a member variable
DOCTEST_MSVC_SUPPRESS_WARNING(26451) // Arithmetic overflow ...
DOCTEST_MSVC_SUPPRESS_WARNING(26444) // Avoid unnamed objects with custom
                                     // construction and dtr...
...
enum Enum
{
    ...
    is_unary = 2 * is_false, // not checked anywhere - used just to distinguish
                             // the types
...
DOCTEST_INTERFACE std::ostream* getTlsOss(); // returns a thread-local
                                             // ostringstream
...
struct Expression_lhs
{
    ...
    // ExpressionDecomposer class uses the left shift operator to capture the
...
struct DOCTEST_INTERFACE TestCase : public TestCaseData
{
    ...
    const char* m_type; // for templated test cases - gets appended to the real
                        // name
    ...
    DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH(26434) // hides a non-virtual
                                                   // function
...
public:
    ...
    DOCTEST_NOINLINE ContextBuilder& operator<<(T& in)
    {
        ...
        // cant construct
        // the object directly in the buffer using placement new - need the
        // <new> header...
...
struct DOCTEST_INTERFACE IReporter
{
    ...
    // argument
    // which has most of the options for the run + a pointer to the stdout
    // stream
...
#define DOCTEST_TEST_CASE_TEMPLATE(dec, T, ...)                            \
    DOCTEST_TEST_CASE_TEMPLATE_IMPL(dec, T,                                \
...
#define DOCTEST_REGISTER_EXCEPTION_TRANSLATOR(signature)                       \
    DOCTEST_REGISTER_EXCEPTION_TRANSLATOR_IMPL(DOCTEST_ANONYMOUS(              \
                                                   _DOCTEST_ANON_TRANSLATOR_), \
                                               signature)
...
#define DOCTEST_ASSERT_IMPLEMENT_1(assert_type, ...)                           \
    DOCTEST_CLANG_SUPPRESS_WARNING_WITH_PUSH(                                  \
        "-Woverloaded-shift-op-parentheses")                                   \
    doctest::detail::decomp_assert(doctest::assertType::assert_type, __FILE__, \
...
DOCTEST_MSVC_SUPPRESS_WARNING(4996) // The compiler encountered a deprecated
                                    // declaration
DOCTEST_MSVC_SUPPRESS_WARNING(4267) // 'var' : conversion from 'x' to 'y',
                                    // possible loss of data
DOCTEST_MSVC_SUPPRESS_WARNING(4706) // assignment within conditional expression
DOCTEST_MSVC_SUPPRESS_WARNING(4512) // 'class' : assignment operator could not
                                    // be generated
DOCTEST_MSVC_SUPPRESS_WARNING(4127) // conditional expression is constant
DOCTEST_MSVC_SUPPRESS_WARNING(4530) // C++ exception handler used, but unwind
                                    // semantics not enabled
DOCTEST_MSVC_SUPPRESS_WARNING(4577) // 'noexcept' used with no exception
                                    // handling mode specified
DOCTEST_MSVC_SUPPRESS_WARNING(4774) // format string expected in argument is not
                                    // a string literal
...
DOCTEST_MSVC_SUPPRESS_WARNING(4640) // construction of local static object is
                                    // not thread-safe
DOCTEST_MSVC_SUPPRESS_WARNING(5039) // pointer to potentially throwing function
                                    // passed to extern C
    ...
    : m_signature(name, file, line)
{
    ...
    {
        if (!matchesAny(m_signature.m_name, s->filters[6], true,
                        s->case_sensitive))
            return;
        if (matchesAny(m_signature.m_name, s->filters[7], false,
                       s->case_sensitive))
            return;
    }
...
DOCTEST_THREAD_LOCAL std::vector<IContextScope*> g_infoContexts; // for logging
                                                                 // with INFO()
...
DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH(4996) // std::uncaught_exception is
                                              // deprecated in C++17
                                  ...
                                  const char* tail = "")
{
    ...
    s << Color::LightGrey << skipPathFromFilename(file)
      << (opt->gnu_file_line ? ":" : "(")
      << (opt->no_line_numbers ? 0 : line) // 0 or the real num depending on the
                                           // option
...
bool parseOptionImpl(int argc, const char* const* argv, const char* pattern,
                     String* value)
{
    // going from the end to the begining and stopping on the first occurance
    // from the end
...
DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH(4007) // 'function' : must be
                                              // 'attribute' - see issue #182
