...
pbrt::Medium* HomogeneusMediumFactory(pbrt::Transform*, const PropertyMap& meta)
{
    ...
    const auto sigADArr =
        meta.getProperty<std::array<double, 3>>("sig_a",
                                                {.0011f, .0024f, .014f});
    const auto sigSDArr =
        meta.getProperty<std::array<double, 3>>("sig_s", {2.55f, 3.21f, 3.77f});
    const auto density =
        static_cast<pbrt::Float>(meta.getProperty<double>("density", 1.0));
    const auto scale =
        static_cast<pbrt::Float>(meta.getProperty<double>("scale", 1.0));
...
pbrt::Medium* HeterogeneusMediumFactory(pbrt::Transform*,
                                        const PropertyMap& meta)
{
    ...
    const auto minDensity =
        static_cast<pbrt::Float>(meta.getProperty<double>("min_density", 1.0));
    const auto maxDensity =
        static_cast<pbrt::Float>(meta.getProperty<double>("max_density", 1.0));
    const auto scale =
        static_cast<pbrt::Float>(meta.getProperty<double>("scale", 1.0));
    const pbrt::Float sigARGB[3] = {static_cast<float>(sigADArr[0]),
                                    static_cast<float>(sigADArr[1]),
...
pbrt::Medium* GridMediumFactory(pbrt::Transform* otw, const PropertyMap& meta)
{
    ...
    {
        BRAYNS_WARN << "PBRTModel: Attempted to add a grid volume whose "
                       "dimensions did not "
    ...
    return new pbrt::GridDensityMedium(sigASpec, sigSSpec, g, nx, ny, nz, *otw,
                                       pbrtDensity.data());
}

template <class ShapeType>
std::shared_ptr<pbrt::AreaLight> createLightForShape(
    std::shared_ptr<ShapeType>& shape, const brayns::TransferFunction& tf,
    MaterialPtr& mat, const float value)
{
    static constexpr float NO_LIGHT_THRESHOLD = 0.1f;
    static constexpr float INTENSITY_THRESHOLD = 0.3f;

    const auto cvalue =
        glm::clamp(static_cast<double>(value), tf.getValuesRange().x,
                   tf.getValuesRange().y);
    float intensity =
        static_cast<float>((cvalue - tf.getValuesRange().x) /
    ...
    return pbrt::CreateDiffuseAreaLight(pbrt::Transform(), nullptr, params,
                                        shape);
}

} // namespace

std::unordered_map<std::string, PBRTModel::MediaFactory>
    PBRTModel::_mediaFactories = {{"homogeneus", HomogeneusMediumFactory},
...
std::vector<std::shared_ptr<pbrt::GeometricPrimitive>> PBRTModel::commitToPBRT(
    ...
    PBRTModel::_createSpheres(const pbrt::Transform& transform)
{
    ...
    for (const auto& sphereList : getSpheres())
    {
        ...
        for (const auto& sphere : sphereList.second)
        {
            ...
            const auto otwS =
                pbrt::Translate(pbrt::Vector3f(sphere.center.x, sphere.center.y,
                                               sphere.center.z));

            std::unique_ptr<pbrt::Transform> otwFinal(new pbrt::Transform(
                pbrt::Matrix4x4::Mul(transform.GetMatrix(), otwS.GetMatrix())));
            ...
            {
                dummyAL =
                    createLightForShape(sphereShape, _transferFunction, mat,
            ...
            result.push_back(std::shared_ptr<pbrt::GeometricPrimitive>(
                new pbrt::GeometricPrimitive(sphereShape, pbrtMat, dummyAL,
                                             dummyMI)));
        }
    }
    ...
    PBRTModel::_createCylinders(const pbrt::Transform& transform)
{
    ...
    for (const auto& cylinderList : getCylinders())
    {
        ...
        for (const auto& cylinder : cylinderList.second)
        {
            ...
            auto pbrtMat =
                pbrt::LookAt(TO_PBRT_P3(cylinder.center),
                             TO_PBRT_P3(cylinder.up), TO_PBRT_V3(rUp));

            const pbrt::Transform cylTrans(pbrtMat);

            std::unique_ptr<pbrt::Transform> otwFinal(new pbrt::Transform(
                pbrt::Matrix4x4::Mul(transform.GetMatrix(),
            ...
            {
                dummyAL =
                    createLightForShape(cylinderShape, _transferFunction, mat,
            ...
            result.push_back(std::shared_ptr<pbrt::GeometricPrimitive>(
                new pbrt::GeometricPrimitive(cylinderShape, pbrtMaterial,
                                             dummyAL, dummyMI)));
        }
    }
...
std::vector<std::shared_ptr<pbrt::GeometricPrimitive>> PBRTModel::_createCones(
    ...
    for (const auto& coneList : getCones())
    {
        ...
        for (const auto& cone : coneList.second)
        {
            ...
            auto pbrtMat = pbrt::LookAt(TO_PBRT_P3(cone.center),
                                        TO_PBRT_P3(cone.up), TO_PBRT_V3(rUp));
            ...
            auto coneShape =
                CreateCustomConeShape(otwFinalPtr, wtoFinalPtr, false, params);

            // Generate a light if we have simulation
            std::shared_ptr<pbrt::AreaLight> dummyAL{nullptr};
            if (!_simulationData.empty())
            {
                dummyAL = createLightForShape<CustomCone>(
                    coneShape, _transferFunction, mat,
                    _simulationData[cone.userData]);
                if (dummyAL)
                    _modelLights.push_back(dummyAL);
            }

            result.push_back(std::shared_ptr<pbrt::GeometricPrimitive>(
                new pbrt::GeometricPrimitive(coneShape, pbrtMaterial, dummyAL,
                                             dummyMI)));
        }
    }
...
std::vector<std::shared_ptr<pbrt::GeometricPrimitive>> PBRTModel::_createMeshes(
    const pbrt::Transform& transform)
{
    Primitives result;

    std::unique_ptr<pbrt::Transform> otw =
        std::make_unique<pbrt::Transform>(transform.GetMatrix(),
                                          transform.GetInverseMatrix());
    std::unique_ptr<pbrt::Transform> wto =
        std::make_unique<pbrt::Transform>(transform.GetInverseMatrix(),
                                          transform.GetMatrix());
    auto otwPtr = otw.get();
    auto wtoPtr = wto.get();
    ...
    for (const auto& meshList : getTriangleMeshes())
    {
        ...
        params.AddInt("indices", std::move(indices),
                      static_cast<int>(mesh.indices.size()) * 3);

        // Verties
        std::unique_ptr<pbrt::Point3f[]> vertices(
            new pbrt::Point3f[mesh.vertices.size()]);
        for (size_t i = 0; i < mesh.vertices.size(); ++i)
            vertices[i] = pbrt::Point3f(mesh.vertices[i].x, mesh.vertices[i].y,
                                        mesh.vertices[i].z);
        params.AddPoint3f("P", std::move(vertices),
        ...
        {
            std::unique_ptr<pbrt::Point2f[]> uvs(
                new pbrt::Point2f[mesh.textureCoordinates.size()]);
        ...
        {
            std::unique_ptr<pbrt::Normal3f[]> normals(
                new pbrt::Normal3f[mesh.normals.size()]);
            for (size_t i = 0; i < mesh.normals.size(); ++i)
                normals[i] =
                    pbrt::Normal3f(mesh.normals[i].x, mesh.normals[i].y,
                                   mesh.normals[i].z);
            params.AddNormal3f("N", std::move(normals),
                               static_cast<int>(mesh.normals.size()));
        }

        auto pbrtMeshList =
            pbrt::CreateTriangleMeshShape(otwPtr, wtoPtr, false, params);
        for (auto& meshObj : pbrtMeshList)
            result.push_back(std::shared_ptr<pbrt::GeometricPrimitive>(
                new pbrt::GeometricPrimitive(meshObj, pbrtMaterial, dummyAL,
                                             dummyMI)));
    }
    ...
    PBRTModel::_createSDFGeometries(const pbrt::Transform& transform)
{
    ...
    std::unique_ptr<pbrt::Transform> otwFinal(
        new pbrt::Transform(transform.GetMatrix(),
                            transform.GetInverseMatrix()));
    std::unique_ptr<pbrt::Transform> wtoFinal(
        new pbrt::Transform(otwFinal->GetInverseMatrix(),
                            otwFinal->GetMatrix()));
    ...
    for (const auto& geometryList : getSDFGeometryData().geometryIndices)
    {
        ...
        for (const auto gemetryIndex : geometryList.second)
        {
            ...
            {
            case SDFType::Sphere:
                shape = std::make_shared<PBRTSDFGeometryShape<SDFType::Sphere>>(
                    otwFinalPtr, wtoFinalPtr, false, &geom,
                    &getSDFGeometryData());
                break;
            case SDFType::Pill:
                shape = std::make_shared<PBRTSDFGeometryShape<SDFType::Pill>>(
                    otwFinalPtr, wtoFinalPtr, false, &geom,
                    &getSDFGeometryData());
                break;
            case SDFType::ConePill:
                shape =
                    std::make_shared<PBRTSDFGeometryShape<SDFType::ConePill>>(
                        otwFinalPtr, wtoFinalPtr, false, &geom,
                        &getSDFGeometryData());
                break;
            case SDFType::ConePillSigmoid:
                shape = std::make_shared<
                    PBRTSDFGeometryShape<SDFType::ConePillSigmoid>>(
                    otwFinalPtr, wtoFinalPtr, false, &geom,
                    &getSDFGeometryData());
                break;
            }
            ...
            if (!_simulationData.empty())
            {
                dummyAL = createLightForShape(shape, _transferFunction, mat,
                                              _simulationData[geom.userData]);
                if (dummyAL)
                    _modelLights.push_back(dummyAL);
            }

            result.push_back(
                std::make_shared<pbrt::GeometricPrimitive>(shape, pbrtMaterial,
                                                           dummyAL, dummyMI));
        }
    }
