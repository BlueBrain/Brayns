...
{
    return type == PBRT_CAMTYPE_REALISTIC ||
...
void PBRTCamera::_createPBRTCamera()
{
    ...
    if(getCurrentType() == PBRT_CAMTYPE_PERSPECTIVE)
        result = pbrt::CreatePerspectiveCamera(params, atr, _film, mockMedium.outside);
    else if(getCurrentType() == PBRT_CAMTYPE_ORTOGRAPHIC)
        result = pbrt::CreateOrthographicCamera(params, atr, _film, mockMedium.outside);
    else if(getCurrentType() == PBRT_CAMTYPE_ENVIRONMENT)
        result = pbrt::CreateEnvironmentCamera(params, atr, _film, mockMedium.outside);
    else if (getCurrentType() == PBRT_CAMTYPE_REALISTIC)
        result = pbrt::CreateRealisticCamera(params, atr, _film, mockMedium.outside);
                     ...
                     const double defaultValue)
{
    ...
    val.get()[0] = static_cast<pbrt::Float>(src.getPropertyOrValue<double>(propName, defaultValue));
...
pbrt::ParamSet PBRTCamera::_camParamstoPbrtParams()
{
    ...
    else if(getCurrentType() == PBRT_CAMTYPE_REALISTIC)
    {
        ...
        lensFileVal.get()[0] = getPropertyOrValue<std::string>(PBRT_CAMPROP_LENSFILE, "");
        ...
        simpleWeightingVal.get()[0] = getPropertyOrValue<bool>(PBRT_CAMPROP_SIMPLEWEIGHTING, true);
        result.AddBool(PBRT_CAMPROP_SIMPLEWEIGHTING, std::move(simpleWeightingVal), 1);
    }
