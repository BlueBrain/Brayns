...
{
    std::shared_ptr<const pbrt::Camera> pbrtCamera =
        renderer.getPBRTCamera()->impl();
    pbrt::Bounds2i pixelBounds = pbrtCamera->film->GetSampleBounds();
    if (renderer.hasProperty(PBRT_INTEGRATORPROP_PIXELBOUNDS))
    {
        auto pbounds = renderer.getProperty<std::array<double, 4>>(
            PBRT_INTEGRATORPROP_PIXELBOUNDS);
        pixelBounds =
            pbrt::Bounds2i(pbrt::Point2i(static_cast<int>(pbounds[0]),
    ...
    auto cossample =
        renderer.getPropertyOrValue<bool>(PBRT_INTEGRATORPROP_COSSAMPLE, true);
    auto nsamples =
        renderer.getPropertyOrValue<int>(PBRT_INTEGRATORPROP_NSAMPLES, 64);

    return std::shared_ptr<pbrt::Integrator>(
        new pbrt::AOIntegrator(cossample, nsamples, pbrtCamera,
                               renderer.getSampler(), pixelBounds));
}
...
std::shared_ptr<pbrt::Integrator> BDPTIntegratorFactory(PBRTRenderer& renderer)
{
    ...
    auto maxdepth =
        renderer.getPropertyOrValue<int>(PBRT_INTEGRATORPROP_MAXDEPTH, 5);
    auto visualizestrategies = renderer.getPropertyOrValue<bool>(
        PBRT_INTEGRATORPROP_VISUALIZESTRATEGIES, false);
    auto visualizeweights =
        renderer.getPropertyOrValue<bool>(PBRT_INTEGRATORPROP_VISUALIZEWEIGHTS,
                                          false);

    if (visualizeweights || visualizestrategies)
        maxdepth = 5;

    auto lightstrategy = renderer.getPropertyOrValue<std::string>(
        PBRT_INTEGRATORPROP_LIGHTSAMPLINGSTRATEGY, "power");

    return std::shared_ptr<pbrt::Integrator>(
        new pbrt::BDPTIntegrator(renderer.getSampler(), pbrtCamera, maxdepth,
                                 visualizestrategies, visualizeweights,
                                 pixelBounds, lightstrategy));
}

std::shared_ptr<pbrt::Integrator> DirectLightingIntegratorFactory(
    PBRTRenderer& renderer)
{
    ...
    auto lightStrategyStr = renderer.getPropertyOrValue<std::string>(
        PBRT_INTEGRATORPROP_LIGHTSTRATEGY, "all");
    ...
    return std::shared_ptr<pbrt::Integrator>(
        new pbrt::DirectLightingIntegrator(strategy, maxdepth, pbrtCamera,
                                           renderer.getSampler(), pixelBounds));
}

std::shared_ptr<pbrt::Integrator> MetropolisLTIntegratorFactory(
    PBRTRenderer& renderer)
{
    std::shared_ptr<const pbrt::Camera> pbrtCamera =
        renderer.getPBRTCamera()->impl();
    auto maxdepth =
        renderer.getPropertyOrValue<int>(PBRT_INTEGRATORPROP_MAXDEPTH, 5);
    auto nbootstrap =
        renderer.getPropertyOrValue<int>(PBRT_INTEGRATORPROP_BOOTSTRAPSAMPLES,
                                         100000);
    auto nchains =
        renderer.getPropertyOrValue<int>(PBRT_INTEGRATORPROP_CHAINS, 1000);
    auto mutationsperpixel =
        renderer.getPropertyOrValue<int>(PBRT_INTEGRATORPROP_MUTATIONSPERPIXEL,
                                         100);
    auto largestepprobability =
        static_cast<pbrt::Float>(renderer.getPropertyOrValue<double>(
            PBRT_INTEGRATORPROP_LARGESTEPPROBABILITY, 0.3));
    auto sigma = static_cast<pbrt::Float>(
        renderer.getPropertyOrValue<double>(PBRT_INTEGRATORPROP_SIGMA, 0.01));

    return std::shared_ptr<pbrt::Integrator>(
        new pbrt::MLTIntegrator(pbrtCamera, maxdepth, nbootstrap, nchains,
                                mutationsperpixel, sigma,
                                largestepprobability));
}

std::shared_ptr<pbrt::Integrator> PathTracerIntegratorFactory(
    PBRTRenderer& renderer)
{
    std::shared_ptr<const pbrt::Camera> pbrtCamera =
        renderer.getPBRTCamera()->impl();
    ...
    auto rrthreshold = static_cast<pbrt::Float>(
        renderer.getPropertyOrValue<double>(PBRT_INTEGRATORPROP_RRTHRESHOLD,
                                            1.));
    auto lightstrategy = renderer.getPropertyOrValue<std::string>(
        PBRT_INTEGRATORPROP_LIGHTSAMPLINGSTRATEGY, "spatial");

    return std::shared_ptr<pbrt::Integrator>(
        new pbrt::PathIntegrator(maxdepth, pbrtCamera, renderer.getSampler(),
...
std::shared_ptr<pbrt::Integrator> SPPMIntegratorFactory(PBRTRenderer& renderer)
{
    ...
    auto maxdepth =
        renderer.getPropertyOrValue<int>(PBRT_INTEGRATORPROP_MAXDEPTH, 5);
    auto niterations =
        renderer.getPropertyOrValue<int>(PBRT_INTEGRATORPROP_NUMITERATIONS, 64);
    auto photonsperiter = renderer.getPropertyOrValue<int>(
        PBRT_INTEGRATORPROP_PHOTONSPERITERATION, -1);
    auto writefreq = renderer.getPropertyOrValue<int>(
        PBRT_INTEGRATORPROP_IMAGEWRITEFREQUENCY, 1 << 31);
    auto radius = static_cast<pbrt::Float>(
        renderer.getPropertyOrValue<double>(PBRT_INTEGRATORPROP_RADIUS, 1.));

    return std::shared_ptr<pbrt::Integrator>(
        new pbrt::SPPMIntegrator(camCopy, niterations, photonsperiter, maxdepth,
                                 radius, writefreq));
}

std::shared_ptr<pbrt::Integrator> VolPathTracerIntegratorFactory(
    PBRTRenderer& renderer)
{
    std::shared_ptr<const pbrt::Camera> pbrtCamera =
        renderer.getPBRTCamera()->impl();
    ...
    return std::shared_ptr<pbrt::Integrator>(
        new pbrt::VolPathIntegrator(maxdepth, pbrtCamera, renderer.getSampler(),
                                    pixelBounds, rrthreshold, lightstrategy));
}

std::shared_ptr<pbrt::Integrator> WhittedIntegratorFactory(
    PBRTRenderer& renderer)
{
    std::shared_ptr<const pbrt::Camera> pbrtCamera =
        renderer.getPBRTCamera()->impl();
    ...
    return std::shared_ptr<pbrt::Integrator>(
        new pbrt::WhittedIntegrator(maxdepth, pbrtCamera, renderer.getSampler(),
                                    pixelBounds));
}
...
std::shared_ptr<pbrt::Integrator> DebugIntegratorFactory(PBRTRenderer& renderer)
{
    std::shared_ptr<const pbrt::Camera> pbrtCamera =
        renderer.getPBRTCamera()->impl();
    pbrt::Bounds2i pixelBounds = pbrtCamera->film->GetSampleBounds();
    return std::make_shared<PBRTDebugIntegrator>(pbrtCamera,
                                                 renderer.getSampler(),
                                                 pixelBounds);
}
...
std::shared_ptr<pbrt::Sampler> ZeroTwoSequenceSamplerFactory(
    PBRTRenderer& renderer)
{
    auto pixelsamples =
        renderer.getPropertyOrValue<int>(PBRT_SAMPLERPROP_PIXELSAMPLES, 16);
    auto dimensions =
        renderer.getPropertyOrValue<int>(PBRT_SAMPLERPROP_DIMENSIONS, 4);
...
std::shared_ptr<pbrt::Sampler> HaltonSamplerFactory(PBRTRenderer& renderer)
{
    ...
    auto pixelsamples =
        renderer.getPropertyOrValue<int>(PBRT_SAMPLERPROP_PIXELSAMPLES, 16);
    auto samplepixelcenter =
        renderer.getPropertyOrValue<bool>(PBRT_SAMPLERPROP_SAMPLEPIXELCENTER,
                                          false);
    return std::shared_ptr<pbrt::Sampler>(
...
{
    auto pixelsamples =
        renderer.getPropertyOrValue<int>(PBRT_SAMPLERPROP_PIXELSAMPLES, 4);
    return std::shared_ptr<pbrt::Sampler>(
    ...
    , _currentSampler(PBRT_SAMPLER_HALTON)
{
    ...
    _renderFactory[PBRT_INTEGRATOR_DIRECTLIGHTING] =
    ...
    _renderFactory[PBRT_INTEGRATOR_VOLUMETRICPATHTRACER] =
        VolPathTracerIntegratorFactory;
    _renderFactory[PBRT_INTEGRATOR_SPROGRESSIVEPHOTOMMAP] =
        SPPMIntegratorFactory;
    ...
    _samplerFactory[PBRT_SAMPLER_ZEROTWOSEQUENCE] =
        ZeroTwoSequenceSamplerFactory;
}
...
void PBRTRenderer::commit()
{
    ...
    if (curSamplerChanged || !_pbrtSampler || isModified())
    {
        ...
        {
            BRAYNS_WARN << "PBRT Renderer: Unknown sampler type "
                        << _currentSampler << ", Falling back to Halton"
                        << std::endl;
            _currentSampler = PBRT_SAMPLER_HALTON;
    ...
    if (rendererChanged || cam.cameraChanged() || isModified())
    {
        ...
        if (!_pbrtRenderer || cam.cameraChanged())
        {
            ...
            {
                BRAYNS_WARN << "PBRT Renderer: Unknown renderer type "
                            << _currentRenderer << ", falling back to Debug"
                            << std::endl;
                _currentRenderer = PBRT_INTEGRATOR_DEBUG;
