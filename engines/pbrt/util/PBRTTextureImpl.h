...
public:
    PBRTTextureImpl(std::unique_ptr<pbrt::TextureMapping2D> mapping,
                    ...
                    bool doTrilinearFilter = true, pbrt::Float maxAnisotropicFilter = 8.f,
...
PBRTTextureImpl<pbrt::RGBSpectrum>::PBRTTextureImpl(
                ...
                bool doTrilinearFilter, pbrt::Float maxAnisotropicFilter,
                pbrt::Float scale, bool gamma)
 : _mapping(std::move(mapping))
{
    ...
    std::unique_ptr<pbrt::RGBSpectrum>
            converted (new pbrt::RGBSpectrum[resx * resy]);
    ...
    for(uint32_t i = 0; i < size; ++i)
    {
        ...
        result[0] = scale * (gamma? pbrt::InverseGammaCorrect(srcVals[0]) : srcVals[0]);
        result[1] = scale * (gamma? pbrt::InverseGammaCorrect(srcVals[1]) : srcVals[1]);
        result[2] = scale * (gamma? pbrt::InverseGammaCorrect(srcVals[2]) : srcVals[2]);
    }
    // Create mip mapping
 ...
 : _mapping(std::move(mapping))
{
    ...
    std::unique_ptr<pbrt::Float>
            converted (new pbrt::Float[resx * resy]);
    ...
    for(uint32_t i = 0; i < size; ++i)
    {
        ...
        // scale + gamma correct
        converted.get()[i] = scale * (gamma? pbrt::InverseGammaCorrect(srcVal) : srcVal);
