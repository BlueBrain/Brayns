...
{
    return T(fabs(static_cast<double>(v.x)), fabs(static_cast<double>(v.y)),
...
inline pbrt::Float sminPoly(const pbrt::Float a, const pbrt::Float b,
                            const pbrt::Float k)
{
    // const float h = glm::clamp(0.5f + 0.5f * (b - a) / k, 0.f, 1.f);
    // return static_cast<pbrt::Float>(glm::lerp(b, a, h) - k * h * (1.f - h));
    const pbrt::Float h =
        pbrt::Clamp(pbrt::Float(0.5) + pbrt::Float(0.5) * (b - a) / k,
                    pbrt::Float(0), pbrt::Float(1));
    return pbrt::Lerp(h, b, a) - k * h * (pbrt::Float(1) - h);
}

inline pbrt::Float sdSphere(const pbrt::Point3f& p, const pbrt::Point3f& c,
                            pbrt::Float r)
{
    return static_cast<pbrt::Float>((p - c).Length() - r);
}

inline pbrt::Float sdCapsule(const pbrt::Point3f& p, const pbrt::Point3f& a,
                             const pbrt::Point3f& b, pbrt::Float r)
{
    const pbrt::Vector3f pa = p - a, ba = b - a;
    const pbrt::Float h = pbrt::Clamp(pbrt::Dot(pa, ba) / pbrt::Dot(ba, ba),
                                      pbrt::Float(0), pbrt::Float(1));
    return (pa - ba * h).Length() - r;
}

inline pbrt::Float sdConePill(const pbrt::Point3f& p, const pbrt::Point3f& p0,
                              const pbrt::Point3f& p1, const pbrt::Float r0,
                              const pbrt::Float r1, const bool useSigmoid)
{
...
inline pbrt::Float calcDistance(const SDFGeometry& primitive,
                                const pbrt::Point3f& p)
{
    ...
    case SDFType::Pill:
        return sdCapsule(p, TO_PBRT_P3(primitive.p0), TO_PBRT_P3(primitive.p1),
                         primitive.r0);
    case SDFType::ConePill:
        return sdConePill(p, TO_PBRT_P3(primitive.p0), TO_PBRT_P3(primitive.p1),
                          primitive.r0, primitive.r1, false);
    case SDFType::ConePillSigmoid:
        return sdConePill(p, TO_PBRT_P3(primitive.p0), TO_PBRT_P3(primitive.p1),
                          primitive.r0, primitive.r1, true);
    }
...
public:
    ...
    pbrt::Bounds3f ObjectBound() const final { return _bounds; }
    /*
    ...
    */
    bool Intersect(const pbrt::Ray& r, pbrt::Float* tHit,
                   ...
                   bool testAlphaTexture = true) const final
    {
        ...
        *tHit = raymarch(ray);
        if (*tHit > 0.f && *tHit < ray.tMax)
        {
            ...
            const pbrt::Normal3f bNormal =
                computeNormal(bPos, calcEpsilon(ray.o, *tHit));
            const pbrt::Point3f pHit = ray(*tHit);
            const pbrt::Vector3f pError =
                pbrt::gamma(5) * pbrt::Abs(static_cast<pbrt::Vector3f>(pHit));
            ...
            pbrt::CoordinateSystem(pbrt::Vector3f(bNormal.x, bNormal.y,
                                                  bNormal.z),
                                   &dpdu, &dpdv);

            *isect = pbrt::SurfaceInteraction(pHit, pError, uv, -ray.d, dpdu,
    ...
    bool IntersectP(const pbrt::Ray& r,
                    bool testAlphaTexture = true) const final
    {
        (void)testAlphaTexture;
        pbrt::Vector3f oErr, dErr;
    ...
    pbrt::Interaction Sample(const pbrt::Point2f& u,
                             pbrt::Float* pdf) const final;
...
private:
        ...
        if (numNeigh > 0)
        {
            ...
            for (uint32_t i = 0; i < numNeigh; ++i)
            {
                ...
                const pbrt::Float blendFactor =
                    pbrt::Lerp(pbrt::Float(PBRT_SDF_BLEND_LERP_FACTOR),
                               std::min(r0, r1), std::max(r0, r1));
     ...
     */
    pbrt::Normal3f computeNormal(const pbrt::Point3f& pos,
                                 const pbrt::Float e) const
    {
        ...
        pbrt::Vector3f temp = pbrt::Normalize(
    ...
    pbrt::Float raymarch(const pbrt::Ray& ray) const
    {
        ...
        for (int i = 0; i < PBRT_MAX_MARCHING_ITERATIONS; ++i)
        {
            ...
            const pbrt::Float currentT =
                bbHit0 + stepLength * static_cast<pbrt::Float>(i + 1);
...
template <>
pbrt::Float PBRTSDFGeometryShape<SDFType::Sphere>::shapeDistance(
    const pbrt::Point3f& p) const
{
    return sdSphere(p, TO_PBRT_P3(_srcGeom->p0), _srcGeom->r0);
}

template <>
pbrt::Float PBRTSDFGeometryShape<SDFType::Pill>::shapeDistance(
    const pbrt::Point3f& p) const
{
    return sdCapsule(p, TO_PBRT_P3(_srcGeom->p0), TO_PBRT_P3(_srcGeom->p1),
                     _srcGeom->r0);
}

template <>
pbrt::Float PBRTSDFGeometryShape<SDFType::ConePill>::shapeDistance(
    const pbrt::Point3f& p) const
{
...
template <>
pbrt::Float PBRTSDFGeometryShape<SDFType::ConePillSigmoid>::shapeDistance(
    const pbrt::Point3f& p) const
{
    return sdConePill(p, TO_PBRT_P3(_srcGeom->p0), TO_PBRT_P3(_srcGeom->p1),
...
{
    const auto h =
        (TO_PBRT_P3(_srcGeom->p1) - TO_PBRT_P3(_srcGeom->p0)).Length();
    return PBRT_M_PI * pbrt::Float(2) * _srcGeom->r0 *
           (_srcGeom->r0 * pbrt::Float(2) + h);
}
...
template <>
pbrt::Interaction PBRTSDFGeometryShape<SDFType::Sphere>::Sample(
    const pbrt::Point2f& u, pbrt::Float* pdf) const
{
    pbrt::Interaction it;
    const auto center =
        pbrt::Point3f(_srcGeom->p0.x, _srcGeom->p0.y, _srcGeom->p0.z);
...
template <>
pbrt::Interaction PBRTSDFGeometryShape<SDFType::Pill>::Sample(
    const pbrt::Point2f& u, pbrt::Float* pdf) const
{
    pbrt::Interaction i;
    ...
    const pbrt::Point3f rayO =
        pbrt::Point3f(bbCenter.x + sample.x, bbCenter.y + sample.y,
                      bbCenter.z + sample.z);
    ...
    pbrt::Float t = 0;
    const pbrt::Float stepLength =
        (radius * 2.0) / static_cast<pbrt::Float>(PBRT_MAX_MARCHING_ITERATIONS);
...
template <>
pbrt::Interaction PBRTSDFGeometryShape<SDFType::ConePill>::Sample(
    const pbrt::Point2f& u, pbrt::Float* pdf) const
{
    pbrt::Interaction i;
...
template <>
pbrt::Interaction PBRTSDFGeometryShape<SDFType::ConePillSigmoid>::Sample(
    const pbrt::Point2f& u, pbrt::Float* pdf) const
{
    pbrt::Interaction i;
