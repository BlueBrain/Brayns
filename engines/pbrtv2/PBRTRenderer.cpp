                                  ...
                                  const Vector3f& defValue)
{
    if (r.hasProperty(prop))
    {
        ...
        float fltP[3] = {static_cast<float>(p[0]), static_cast<float>(p[1]),
...
{
    const auto maxDist =
        getFloat(renderer, PBRT_SURFACEINTEGRATOR_PROP_MAXDIST, 99999999.9f);
    const auto nsamples =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_NSAMPLES, 2048);
...
pbrt::SurfaceIntegrator* DiffusePRTIntegratorFactory(PBRTRenderer& renderer)
{
    ...
    const auto nsamples =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_NSAMPLES, 4096);
    return new pbrt::DiffusePRTIntegrator(lmax, nsamples);
}

pbrt::SurfaceIntegrator* DipoleSubsurfaceIntegratorFactory(
    PBRTRenderer& renderer)
{
    const auto maxDepth =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_MAXDEPTH, 5);
    const auto maxError =
        getFloat(renderer, PBRT_SURFACEINTEGRATOR_PROP_MAXERROR, .05f);
    const auto minDist =
        getFloat(renderer, PBRT_SURFACEINTEGRATOR_PROP_MINSAMPLEDISTANCE, .25f);
    const auto pointsfile = renderer.getPropertyOrValue<std::string>(
        PBRT_SURFACEINTEGRATOR_PROP_POINTSFILE, "");
    return new pbrt::DipoleSubsurfaceIntegrator(maxDepth, maxError, minDist,
                                                pointsfile);
}
...
pbrt::SurfaceIntegrator* DirectlightingIntegratorFactory(PBRTRenderer& renderer)
{
    const auto maxDepth =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_MAXDEPTH, 5);
    const auto strategy = renderer.getPropertyOrValue<std::string>(
        PBRT_SURFACEINTEGRATOR_PROP_STRATEGY, "all");
    ...
    if (strategy == std::string("all"))
        ...
        throw std::runtime_error(
            "PBRTEngine -> DirectLighting surface integrator: "
...
pbrt::SurfaceIntegrator* GlossyPRTIntegratorFactory(PBRTRenderer& renderer)
{
    ...
    const auto nsamples =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_NSAMPLES, 4096);
    const auto Kd =
        getSpectrum(renderer, PBRT_SURFACEINTEGRATOR_PROP_KD, {.5f, .5f, .5f});
    const auto Ks = getSpectrum(renderer, PBRT_SURFACEINTEGRATOR_PROP_KS,
                                {.25f, .25f, .25f});
    const auto roughness =
        getFloat(renderer, PBRT_SURFACEINTEGRATOR_PROP_ROUGHNESS, 0.1f);
...
{
    const auto nLightPaths =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_NLIGHTS, 64);
    const auto nLightSets =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_NSETS, 4);
    const auto rrThresh =
        getFloat(renderer, PBRT_SURFACEINTEGRATOR_PROP_RRTHRESHOLD, .0001f);
    const auto maxDepth =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_MAXDEPTH, 5);
    const auto glimit =
        getFloat(renderer, PBRT_SURFACEINTEGRATOR_PROP_GLIMIT, 10.f);
    const auto gatherSamples =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_GATHERSAMPLES, 16);
    return new pbrt::IGIIntegrator(nLightPaths, nLightSets, rrThresh, maxDepth,
                                   glimit, gatherSamples);
}

pbrt::SurfaceIntegrator* IrradianceCacheIntegratorFactory(
    PBRTRenderer& renderer)
{
    const auto minWeight =
        getFloat(renderer, PBRT_SURFACEINTEGRATOR_PROP_MINWEIGHT, 0.5f);
    const auto minSpacing =
        getFloat(renderer, PBRT_SURFACEINTEGRATOR_PROP_MINPIXELSPACING, 2.5f);
    const auto maxSpacing =
        getFloat(renderer, PBRT_SURFACEINTEGRATOR_PROP_MAXPIXELSPACING, 15.f);
    const auto maxAngle =
        getFloat(renderer, PBRT_SURFACEINTEGRATOR_PROP_MAXANGLEDIFFERENCE,
                 10.f);
    const auto maxSpecularDepth =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_MAXSPECULARDEPTH, 5);
    const auto maxIndirectDepth =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_MAXINDIRECTDEPTH, 3);
    const auto nsamples =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_NSAMPLES, 4096);
    return new pbrt::IrradianceCacheIntegrator(minWeight, minSpacing,
                                               maxSpacing, maxAngle,
                                               maxSpecularDepth,
...
{
    const auto maxDepth =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_MAXDEPTH, 1);
...
{
    const auto nCaustic =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_CAUSTICPHOTONS, 20000);
    const auto nIndirect =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_INDIRECTPHOTONS, 100000);
    const auto nUsed = getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_NUSED, 50);
    const auto maxSpecularDepth =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_MAXSPECULARDEPTH, 5);
    const auto maxPhotonDepth =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_MAXPHOTONDEPTH, 5);
    const auto finalGather = renderer.getPropertyOrValue<bool>(
        PBRT_SURFACEINTEGRATOR_PROP_FINALGATHER, true);
    const auto gatherSamples =
        getInt(renderer, PBRT_SURFACEINTEGRATOR_PROP_FINALGATHERSAMPLES, 32);
    const auto maxDist =
        getFloat(renderer, PBRT_SURFACEINTEGRATOR_PROP_MAXDIST, .1f);
    const auto gatherAngle =
        getFloat(renderer, PBRT_SURFACEINTEGRATOR_PROP_GATHERANGLE, 10.f);
    return new pbrt::PhotonIntegrator(nCaustic, nIndirect, nUsed,
...
{
    const auto filename = renderer.getPropertyOrValue<std::string>(
        PBRT_SURFACEINTEGRATOR_PROP_FILENAME, "probes.out");
...
{
    const auto stepSize =
        getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_STEPSIZE, 1.f);
    return new pbrt::EmissionIntegrator(stepSize);
}

pbrt::VolumeIntegrator* FlourescenceSingleIntegratorFactory(
    PBRTRenderer& renderer)
{
    const auto stepSize =
        getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_STEPSIZE, 1.f);
...
pbrt::VolumeIntegrator* FlourescenceSingleRWLIntegratorFactory(
    PBRTRenderer& renderer)
{
    const auto stepSize =
        getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_STEPSIZE, 1.f);
...
pbrt::VolumeIntegrator* SensorIntegratorFactory(PBRTRenderer& renderer)
{
    const auto stepSize =
        getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_STEPSIZE, 1.f);
    const auto photonCount =
        getInt(renderer, PBRT_VOLUMEINTEGRATOR_PROP_PHOTONCOUNT, 1000);
    return new pbrt::SensorIntegrator(stepSize,
                                      static_cast<uint64_t>(photonCount));
}

pbrt::VolumeIntegrator* SingleScatteringIntegratorFactory(
    PBRTRenderer& renderer)
{
    const auto stepSize =
        getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_STEPSIZE, 1.f);
...
pbrt::VolumeIntegrator* VolBDPathIntegratorFactory(PBRTRenderer& renderer)
{
    const auto stepSize =
        getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_STEPSIZE, 1.f);
    const auto maxEyeDepth =
        getInt(renderer, PBRT_VOLUMEINTEGRATOR_PROP_EYEDEPTH, 3);
    const auto maxlightDepth =
        getInt(renderer, PBRT_VOLUMEINTEGRATOR_PROP_LIGHTDEPTH, 3);
    return new pbrt::VolumeBDPTIntegrator(stepSize,
                                          static_cast<uint64_t>(maxEyeDepth),
...
pbrt::VolumeIntegrator* VPLIntegratorFactory(PBRTRenderer& renderer)
{
    const auto stepSize =
        getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_STEPSIZE, 1.f);
    const auto nLightPaths =
        getInt(renderer, PBRT_VOLUMEINTEGRATOR_PROP_NLGIHTS, 4);
    const auto nLightSets =
        getInt(renderer, PBRT_VOLUMEINTEGRATOR_PROP_NSETS, 4);
    const auto glimit =
        getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_GLIMIT, 1000.f);
...
pbrt::VolumeIntegrator* VSDFDSIntegratorFactory(PBRTRenderer& renderer)
{
    ...
    return new pbrt::VSDForwardDirectIntegrator(vsdDataDirectory, pshFileName,
                                                shift);
}
...
pbrt::VolumeIntegrator* VSDFLSIntegratorFactory(PBRTRenderer& renderer)
{
    ...
    shift.y = getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_YSHIFT, 0.f);
    shift.z = getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_ZSHIFT, 0.f);
    return new pbrt::VSDForwardLinearIntegrator(vsdDataDirectory, pshFileName,
                                                shift);
}
...
pbrt::VolumeIntegrator* VSDFSSIntegratorFactory(PBRTRenderer& renderer)
{
    ...
    shift.y = getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_YSHIFT, 0.f);
    shift.z = getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_ZSHIFT, 0.f);
    return new pbrt::VSDForwardScatteringIntegrator(vsdDataDirectory,
                                                    pshFileName, shift);
}
...
pbrt::VolumeIntegrator* VSDLinearSpriteIntegratorFactory(PBRTRenderer& renderer)
{
    ...
    shift.y = getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_YSHIFT, 0.f);
    shift.z = getFloat(renderer, PBRT_VOLUMEINTEGRATOR_PROP_ZSHIFT, 0.f);
    return new pbrt::VSDLinearSpriteIntegrator(vsdDataDirectory, pshFileName,
                                               shift);
}
...
{
    const PBRTCamera& pbrtCam =
        static_cast<const PBRTCamera&>(*renderer.getPBRTCamera());
    ...
    const auto method =
        renderer.getPropertyOrValue<std::string>(PBRT_SAMPLERPROP_METHOD,
                                                 "contrast");

    if (method != "contrast" && method != "shapeid")
        throw std::runtime_error(
            "PBRTRenderer -> AdaptiveSamplerFactory: Available method property "
            "values are \"contrast\" and \"shapeid\", but it was given "
            "\"" +
            method + "\"");

    return new pbrt::AdaptiveSampler(xstart, xend, ystart, yend, minsamp,
                                     maxsamp, method,
                                     pbrtCam.impl()->shutterOpen,
                                     pbrtCam.impl()->shutterClose);
}
...
pbrt::Sampler* StratifiedSamplerFactory(PBRTRenderer& renderer)
{
    ...
    const auto jitter =
        renderer.getPropertyOrValue<bool>(PBRT_SAMPLERPROP_JITTER, true);
    int xstart, xend, ystart, yend;
    pbrtCam.impl()->film->GetSampleExtent(&xstart, &xend, &ystart, &yend);
    const auto xsamp = getInt(renderer, PBRT_SAMPLERPROP_XSAMPLES, 2);
    const auto ysamp = getInt(renderer, PBRT_SAMPLERPROP_YSAMPLES, 2);
    return new pbrt::StratifiedSampler(xstart, xend, ystart, yend, xsamp, ysamp,
                                       jitter, pbrtCam.impl()->shutterOpen,
                                       pbrtCam.impl()->shutterClose);
}
...
{
    _surfaceIntegratorFactory[PBRT_SURFACE_INTEGRATOR_AMBIENTOCCLUSION] =
        AOIntegratorFactory;
    _surfaceIntegratorFactory[PBRT_SURFACE_INTEGRATOR_DIRECTLIGHTING] =
        DirectlightingIntegratorFactory;
    _surfaceIntegratorFactory[PBRT_SURFACE_INTEGRATOR_PATHTRACER] =
        PathIntegratorFactory;
    _surfaceIntegratorFactory[PBRT_SURFACE_INTEGRATOR_PHOTOMMAP] =
        PhotonMapIntegratorFactory;
    _surfaceIntegratorFactory[PBRT_SURFACE_INTEGRATOR_IRRADIANCACHE] =
        IrradianceCacheIntegratorFactory;
    _surfaceIntegratorFactory[PBRT_SURFACE_INTEGRATOR_IGI] =
        IGIIntegratorFactory;
    _surfaceIntegratorFactory[PBRT_SURFACE_INTEGRATOR_DIPOLESUBSURFACE] =
        DipoleSubsurfaceIntegratorFactory;
    _surfaceIntegratorFactory[PBRT_SURFACE_INTEGRATOR_RADIANCEPROBE] =
        RadianceProbesIntegratorFactory;
    _surfaceIntegratorFactory[PBRT_SURFACE_INTEGRATOR_DIFFUSEPRT] =
        DiffusePRTIntegratorFactory;
    _surfaceIntegratorFactory[PBRT_SURFACE_INTEGRATOR_GLOSSYPRT] =
        GlossyPRTIntegratorFactory;
    _surfaceIntegratorFactory[PBRT_SURFACE_INTEGRATOR_DEBUG] =
        DebugSurfaceIntegratorFactory;

    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_SINGLESCATTERING] =
        SingleScatteringIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_VPL] = VPLIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_SINGLEFLOURESCENCE] =
        FlourescenceSingleIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_SINGLEFLOURESCENCEWL] =
        FlourescenceSingleRWLIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_SENSOR] =
        SensorIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_VSDFDS] =
        VSDFDSIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_VSDFLS] =
        VSDFLSIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_VSDFSS] =
        VSDFSSIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_VSDBDG] =
        VSDBDGIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_VSDBLG] =
        VSDBLGIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_VSDBSG] =
        VSDBSGIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_VSDSPRITELINEAR] =
        VSDLinearSpriteIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_VSDSCATTERING] =
        VSDScatteringIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_EMISSION] =
        EmissionIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_PATH] =
        VolPathIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_BDPT] =
        VolBDPathIntegratorFactory;
    _volumeIntegratorFactory[PBRT_VOLUME_INTEGRATOR_DEBUG] =
        DebugVolumeIntegratorFactory;
...
void PBRTRenderer::render(FrameBufferPtr frameBuffer)
{
    ...
    if (pbrtSceneImpl)
    {
        ...
        PBRTFrameBuffer* pbrtFB =
            static_cast<PBRTFrameBuffer*>(frameBuffer.get());
        ...
        BRAYNS_WARN << "PBRTRenderer: PBRT Scene is null, skipping rendernig!"
                    << std::endl;
}
...
void PBRTRenderer::manualCommit(const Vector2i& res)
{
    ...
    const auto propSamplerName =
        getPropertyOrValue(PBRT_SAMPLERPROP_TYPE, _currentSampler);
    const auto propSurfIntegrator =
        getPropertyOrValue<std::string>(PBRT_SURFACEINTEGRATOR_PROP_TYPE,
                                        _currentSurfIntegrator);
    const auto propVolIntegrator =
        getPropertyOrValue<std::string>(PBRT_VOLUMEINTEGRATOR_PROP_TYPE,
                                        _currentVolIntegrator);
    ...
    auto surfaceFactoryIt =
        _surfaceIntegratorFactory.find(_currentSurfIntegrator);
    if (surfaceFactoryIt == _surfaceIntegratorFactory.end())
    {
        BRAYNS_WARN << "PBRT Renderer: Unknown surface integrator "
                    << _currentSurfIntegrator << ", falling back to Debug"
                    << std::endl;
        _currentSurfIntegrator = PBRT_SURFACE_INTEGRATOR_DEBUG;
        surfaceFactoryIt =
            _surfaceIntegratorFactory.find(_currentSurfIntegrator);
    }
    pbrt::SurfaceIntegrator* surfaceIntegrator =
        surfaceFactoryIt->second(*this);
    ...
    {
        BRAYNS_WARN << "PBRT Renderer: Unknown volume integrator "
                    << _currentVolIntegrator << ", falling back to Debug"
                    << std::endl;
        _currentVolIntegrator = PBRT_VOLUME_INTEGRATOR_DEBUG;
    ...
    _renderer =
        std::make_shared<pbrt::SamplerRenderer>(sampler, cam.impl(),
                                                surfaceIntegrator,
                                                volumeIntegrator, false);

    _scene->visitModels([](Model& m) { m.markInstancesDirty(); });
    _scene->markModified();
    _scene->commit();
