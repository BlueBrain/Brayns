 ...
 */
pbrt::Reference<pbrt::Texture<float>>
constFloatTexture(const Material& m,
                  const std::string& prop,
                  const double defaultValue = 0.)
{
    const auto val = static_cast<float>(m.getPropertyOrValue<double>(prop, defaultValue));
 ...
 */
pbrt::Reference<pbrt::Texture<float>>
constFloatTextureOrNull(const Material& m,
                        const std::string& prop)
{
 ...
 * @return a shared_ptr with the constant texture
 */
pbrt::Reference<pbrt::Texture<pbrt::Spectrum>>
constSpectrumTexture(const Material& m,
                     const std::string& prop,
...
constSpectrumTextureOrNull(const Material& m,
                           const std::string& prop)
{
    if(m.hasProperty(prop))
    {
 ...
 */
pbrt::Reference<pbrt::Texture<pbrt::Spectrum>>
braynsTextureToPbrtTexture(const Material& m,
                           ...
                           const std::array<float, 3>& defaultValue)
{
    ...
    if(m.hasTexture(type))
    {
        ...
        rawPtr = new PBRTTextureImpl<pbrt::Spectrum>(new pbrt::UVMapping2D(), *texPtr);
    }
    else // Otherwise, create a texture with a constant diffuse color
    {
        const float rgbValues[] =
        {
          static_cast<float>(defaultValue[0]),
          static_cast<float>(defaultValue[1]),
          static_cast<float>(defaultValue[2])
        };

        rawPtr = new pbrt::ConstantTexture<pbrt::Spectrum>(pbrt::Spectrum::FromRGB(rgbValues));
    }
 ...
 */
pbrt::Reference<pbrt::Texture<pbrt::Spectrum>> braynsTextureToPbrtTextureOrNull(const Material& m,
                                                                                const TextureType type)
{
    if(m.hasTexture(type))
    {
        ...
        if(texPtr != nullptr) // If we have color texture, use it
            return pbrt::Reference<pbrt::Texture<pbrt::Spectrum>>(
                new PBRTTextureImpl<pbrt::Spectrum>(new pbrt::UVMapping2D(), *texPtr));
    }
...
template<>
pbrt::Material* PBRTMaterial::_createPBRTMaterial<pbrt::MeasuredMaterial>() const
{
     // BUMP MAP TEXTURE
    std::shared_ptr<Texture2D> bumpTexPtr {nullptr};
    pbrt::Reference<pbrt::Texture<float>> bumpmap (nullptr);
    ...
    return new pbrt::MeasuredMaterial(getPropertyOrValue<std::string>("measuredFilename", ""),
                                      bumpmap);
}
...
template<>
pbrt::Material* PBRTMaterial::_createPBRTMaterial<pbrt::ShinyMetalMaterial>() const
{
    // KD TEXTURE
    auto Kr = braynsTextureToPbrtTexture(*this, TextureType::diffuse,
                                        {static_cast<float>(getDiffuseColor().r),
                                         static_cast<float>(getDiffuseColor().g),
                                         static_cast<float>(getDiffuseColor().b)});

    // KS TEXTURE
    auto Ks = constSpectrumTexture(*this, "shinymetalKs", {getSpecularColor().r,
                                                         getSpecularColor().g,
                                                         getSpecularColor().b});
...
pbrt::Material* PBRTMaterial::_createPBRTMaterial<pbrt::SubsurfaceMaterial>() const
{
    ...
    float sigARGB[3] = {static_cast<float>(0.0011),
                             static_cast<float>(0.0024),
                             static_cast<float>(0.014)};
    float sigSRGB[3] = {static_cast<float>(2.55),
                             static_cast<float>(3.21),
    ...
    // SCALE VALUE
    auto scale = static_cast<float>(getPropertyOrValue<double>("subsurfScale", 1.));
    ...
    return new pbrt::SubsurfaceMaterial(scale, Kr, sigATexture, sigSTexture, index, bumpmap);
}
...
pbrt::Material* PBRTMaterial::_createPBRTMaterial<pbrt::TranslucentMaterial>() const
{
    ...
    return new pbrt::TranslucentMaterial(Kd, Ks, roughness, reflect, transmit, bumpmap);
}
...
pbrt::Material* PBRTMaterial::_createPBRTMaterial<pbrt::UberMaterial>() const
{
    ...
    return new pbrt::UberMaterial(Kd, Ks, Kr, Kt, roughness, opacity, index, bumpmap);
}
