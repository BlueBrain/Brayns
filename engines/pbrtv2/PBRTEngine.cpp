...
void PBRTEngine::_createCameras()
{
    ...
    common.setProperty(
        {PBRT_CAMPROP_SHUTTERCLOSE, 1., {"Shutter close value"}});
    ...
    common.setProperty(
        {PBRT_CAMPROP_SCREENWINDOW, cropWindow, {"Window crop"}});
...
void PBRTEngine::_createRenderers()
{
    ...
    // SAMPLER SETTINGS
    props.setProperty({PBRT_SAMPLERPROP_TYPE,
                       PBRT_SAMPLER_HALTON,
                       {"Type of numeric sampler to use"}});
    props.setProperty(
        {PBRT_SAMPLERPROP_PIXELSAMPLES, 4, {"Number of samples per pixel"}});
    props.setProperty({PBRT_SAMPLERPROP_METHOD,
                       std::string("contrast"),
                       {"Sampling method (Adaptive)"}});
    props.setProperty(
        {PBRT_SAMPLERPROP_MINSAMPLES, 4, {"Min number of samples (Adaptive)"}});
    props.setProperty({PBRT_SAMPLERPROP_MAXSAMPLES,
                       32,
                       {"Max number of samples (Adaptive)"}});
    props.setProperty({PBRT_SAMPLERPROP_JITTER,
                       true,
                       {"Wether to jitter sampling (Stratified)"}});
    props.setProperty({PBRT_SAMPLERPROP_XSAMPLES,
                       4,
                       {"Number of horizontal samples (Stratified)"}});
    props.setProperty({PBRT_SAMPLERPROP_YSAMPLES,
                       4,
                       {"Number of vertical samples (Stratified)"}});

    // SURFACE INTEGRATOR PROPERTIES
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_TYPE,
                       PBRT_SURFACE_INTEGRATOR_DEBUG,
                       {"Surface integrator class"}});
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_POINTSFILE,
                       std::string(""),
                       {"Dipole subsurface integrator - path to points file"}});
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_MAXERROR,
                       0.05,
                       {"Dipole subsurface integrator - max error threshold"}});
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_GATHERSAMPLES,
                       16,
                       {"Instant GI integrator - num of samples gathering"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_MAXINDIRECTDEPTH,
         3,
         {"Irradiance cache integrator - max indirect path depth"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_NUSED,
         50,
         {"Photon map integrator - Number of photons for density est."}});
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_LMAX,
                       4,
                       {"Glossy PRT integrator - Max SH band number"}});
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_FILENAME,
                       std::string(""),
                       {"Radiance probes integrator - path to probes file"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_CAUSTICPHOTONS,
         20000,
         {"Photon map integrator - number of photons for caustics"}});
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_NLIGHTS,
                       64,
                       {"Instant GI integrator - Number of light paths"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_MAXSPECULARDEPTH,
         5,
         {"Irradiance cache, Photon map - Max specular ray depth"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_GLIMIT,
         10.0,
         {"Instant GI integrator - Max value for G (geometric coupling)"}});
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_MAXPHOTONDEPTH,
                       5,
                       {"Photon map integrator - Max phton path depth"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_FINALGATHER,
         true,
         {"Photon map integrator - Wether to perform final gather pass"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_MINSAMPLEDISTANCE,
         0.25,
         {"Dipole subsurface integrator - minimun distance between samples"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_NSAMPLES,
         2048,
         {"Ambient occlussion, Diffuse PRT, Glossy PRT, Irradiance Cache - "
          "Number of rays to determine radiance"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_STRATEGY,
         std::string("all"),
         {"Direct lighting integrator - Light sampling strategy (all|one)"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_NSETS,
         4,
         {"Instant GI integrator - Num of independent virtual light sets"}});
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_RRTHRESHOLD,
                       0.0001,
                       {"Instant GI integrator - Russian Roulette threshold"}});
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_GATHERANGLE,
                       10.0,
                       {"Photon map integrator - Cone spread angle for IS "
                        "photon gathering"}});
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_MINWEIGHT,
                       0.5,
                       {"Irradiance cache integrator - Minimun weight to "
                        "interpolate samples"}});
    const std::array<double, 3> KDparam = {0.5, 0.5, 0.5};
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_KD,
                       KDparam,
                       {"Glossy prt integrator - Diffuse coefficent"}});
    const std::array<double, 3> KSparam = {0.25, 0.25, 0.25};
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_KS,
                       KSparam,
                       {"Glossy prt integrator - Specular coefficent"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_MAXDEPTH,
         5,
         {"Dipole subsurface, Direct lighting, Instant GI, Path, Whitted - "
          "Max ray depth"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_MAXDIST,
         9999999.9,
    ...
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_FINALGATHERSAMPLES,
         32,
         {"Photon map integrator - Final gathering pass sample count"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_MINPIXELSPACING,
         2.5,
         {"Irradiance cache integrator - minimum pixel spacing"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_MAXPIXELSPACING,
         15.0,
         {"Irradiance cache integrator - maximum pixel spacing"}});
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_INDIRECTPHOTONS,
                       100000,
                       {"Photon map integrator - Indirect photons count"}});
    props.setProperty({PBRT_SURFACEINTEGRATOR_PROP_ROUGHNESS,
                       0.1,
                       {"Glossy prt integrator - Surface roughness"}});
    props.setProperty(
        {PBRT_SURFACEINTEGRATOR_PROP_MAXANGLEDIFFERENCE,
         10.0,
         {"Irradiacne cache integrator - Max angle difference between "
          "normal and irradiance look up point"}});

    // VOLUME INTEGRATOR PROPERTIES
    props.setProperty({PBRT_VOLUMEINTEGRATOR_PROP_TYPE,
                       PBRT_VOLUME_INTEGRATOR_DEBUG,
                       {"Volume integrator class"}});
    props.setProperty(
        {PBRT_VOLUMEINTEGRATOR_PROP_LIGHTDEPTH,
         3,
    ...
    props.setProperty({PBRT_VOLUMEINTEGRATOR_PROP_EYEDEPTH,
                       3,
                       {"Volumetric BD path integrator - Max eye ray depth"}});
    props.setProperty(
        {PBRT_VOLUMEINTEGRATOR_PROP_NSETS, 4, {"VPL volume integrator"}});
    props.setProperty(
        {PBRT_VOLUMEINTEGRATOR_PROP_NLGIHTS, 4, {"VPL volume integrator"}});
    props.setProperty(
        {PBRT_VOLUMEINTEGRATOR_PROP_GLIMIT, 1000.0, {"VPL volume integrator"}});
    props.setProperty({PBRT_VOLUMEINTEGRATOR_PROP_PHOTONCOUNT,
                       1000,
                       {"Sensor volume integrator"}});
    props.setProperty({PBRT_VOLUMEINTEGRATOR_PROP_VSDDATADIRECTORY,
                       std::string(""),
                       {"VSDFDS, VSDFLS, VSDFSS, VSD linear sprite"}});
    props.setProperty({PBRT_VOLUMEINTEGRATOR_PROP_PSHFILE,
                       std::string(""),
                       {"VSDFDS, VSDFLS, VSDFSS, VSD linear sprite"}});
    props.setProperty({PBRT_VOLUMEINTEGRATOR_PROP_XSHIFT,
                       0.0,
                       {"VSDFDS, VSDFLS, VSDFSS, VSD linear sprite"}});
    props.setProperty({PBRT_VOLUMEINTEGRATOR_PROP_YSHIFT,
                       0.0,
                       {"VSDFDS, VSDFLS, VSDFSS, VSD linear sprite"}});
    props.setProperty({PBRT_VOLUMEINTEGRATOR_PROP_ZSHIFT,
                       0.0,
                       {"VSDFDS, VSDFLS, VSDFSS, VSD linear sprite"}});
    props.setProperty({PBRT_VOLUMEINTEGRATOR_PROP_DEPTH,
                       3,
                       {"Volumetric Path integrator - Max ray depth"}});
    props.setProperty(
        {PBRT_VOLUMEINTEGRATOR_PROP_STEPSIZE,
         1.0,
