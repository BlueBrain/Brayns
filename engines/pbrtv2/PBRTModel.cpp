...
namespace brayns
{
pbrt::VolumeRegion* HomogeneusVolumeFactory(pbrt::Transform* otw,
                                            const PropertyMap& meta)
{
    const auto g = static_cast<float>(meta.getProperty<double>("g", 0.0));
    const auto sigADArr =
        meta.getProperty<Vector3d>("sig_a", {.0011f, .0024f, .014f});
    const auto sigSDArr =
        meta.getProperty<Vector3d>("sig_s", {2.55f, 3.21f, 3.77f});
    const auto density =
        static_cast<float>(meta.getProperty<double>("density", 1.0));
    ...
    pbrt::Point p0(static_cast<float>(p0Arr[0]), static_cast<float>(p0Arr[1]),
                   static_cast<float>(p0Arr[2]));
    pbrt::Point p1(static_cast<float>(p1Arr[0]), static_cast<float>(p1Arr[1]),
                   static_cast<float>(p1Arr[2]));

    return new pbrt::HomogeneousVolumeDensity(sigASpec, sigSSpec, density, g,
                                              LeSpec, pbrt::BBox(p0, p1), *otw);
}

pbrt::VolumeRegion* HeterogeneusVolumeFactory(pbrt::Transform* otw,
                                              const PropertyMap& meta)
{
    const auto g = static_cast<float>(meta.getProperty<double>("g", 0.0));
    const auto sigADArr =
    ...
    const auto minDensity =
        static_cast<float>(meta.getProperty<double>("min_density", 0.0));
    const auto maxDensity =
        static_cast<float>(meta.getProperty<double>("max_density", 1.0));
    const auto LeDArr = meta.getProperty<Vector3d>("Le", {1., 1., 1.});
    const auto p0Arr = meta.getProperty<Vector3d>("p0", {0., 0., 0.});
    const auto p1Arr = meta.getProperty<Vector3d>("p1", {1., 1., 1.});
    ...
    return new pbrt::HeterogeneousVolumeDensity(sigASpec, sigSSpec, minDensity,
                                                maxDensity, g, LeSpec,
                                                pbrt::BBox(p0, p1), *otw);
}

pbrt::VolumeRegion* GridVolumeFactory(pbrt::Transform* otw,
                                      const PropertyMap& meta)
{
    const auto g = static_cast<float>(meta.getProperty<double>("g", 0.0));
    const auto sigADArr =
    ...
    const auto sigSDArr =
        meta.getProperty<Vector3d>("sig_s", {2.55f, 3.21f, 3.77f});
    const auto density =
        meta.getProperty<std::vector<float>>("density", std::vector<float>());
    ...
    if (gridType == std::string("grid"))
        result =
            new pbrt::VolumeGrid(sigASpec, sigSSpec, g, LeSpec,
                                 pbrt::BBox(p0, p1), *otw,
    ...
    {
        result =
            new pbrt::VSDVolumeGrid(sigASpec, sigSSpec, g, LeSpec,
                                    pbrt::BBox(p0, p1), *otw,
                                    static_cast<uint64_t>(nx),
                                    static_cast<uint64_t>(ny),
                                    static_cast<uint64_t>(nz), density.data());
...
pbrt::VolumeRegion* FluorescentVolumeFactory(pbrt::Transform* otw,
                                             const PropertyMap& meta)
{
    const auto p0Arr = meta.getProperty<Vector3d>("p0", {0., 0., 0.});
    const auto p1Arr = meta.getProperty<Vector3d>("p1", {1., 1., 1.});
    ...
    const auto epsilon =
        static_cast<float>(meta.getProperty<double>("epsilon"));
    ...
    return new pbrt::FluorescentVolumeDensity(pbrt::BBox(p0, p1), *otw, fex,
                                              fem, epsilon, c, yield, gf);
}

pbrt::VolumeRegion* FluorescentAnnotatedVolumeFactory(pbrt::Transform* otw,
                                                      const PropertyMap& meta)
{
    const auto p0Arr = meta.getProperty<Vector3d>("p0", {0., 0., 0.});
    const auto p1Arr = meta.getProperty<Vector3d>("p1", {1., 1., 1.});
    ...
    return new pbrt::FluorescentAnnotatedVolumeGrid(pbrt::BBox(p0, p1), *otw,
                                                    fex, fem, epsilon, c, yield,
                                                    gf, nx, ny, nz, indices,
                                                    ntags);
}

pbrt::VolumeRegion* FluorescentBinaryVolumeFactory(pbrt::Transform* otw,
                                                   const PropertyMap& meta)
{
    const auto p0Arr = meta.getProperty<Vector3d>("p0", {0., 0., 0.});
    const auto p1Arr = meta.getProperty<Vector3d>("p1", {1., 1., 1.});
    ...
    return new pbrt::FluorescentBinaryVolumeGrid(pbrt::BBox(p0, p1), *otw, nx,
                                                 ny, nz, density, fex, fem,
                                                 epsilon, c, yield, gf);
}

pbrt::VolumeRegion* FluorescentGridVolumeFactory(pbrt::Transform* otw,
                                                 const PropertyMap& meta)
{
    const auto p0Arr = meta.getProperty<Vector3d>("p0", {0., 0., 0.});
    const auto p1Arr = meta.getProperty<Vector3d>("p1", {1., 1., 1.});
    ...
    return new pbrt::FluorescentGridDensity(pbrt::BBox(p0, p1), *otw, nx, ny,
                                            nz, data, fex, fem, epsilon, c,
                                            yield, gf);
}

pbrt::VolumeRegion* FluorescentScatteringVolumeFactory(pbrt::Transform* otw,
                                                       const PropertyMap& meta)
{
    const auto p0Arr = meta.getProperty<Vector3d>("p0", {0., 0., 0.});
    const auto p1Arr = meta.getProperty<Vector3d>("p1", {1., 1., 1.});
    ...
    const auto mweight =
        static_cast<float>(meta.getProperty<double>("mweight"));
    const auto epsilon =
        static_cast<float>(meta.getProperty<double>("epsilon"));
    const auto c = static_cast<float>(meta.getProperty<double>("c"));
    const auto yield = static_cast<float>(meta.getProperty<double>("yield"));
    ...
    const auto gf = static_cast<float>(meta.getProperty<double>("gf"));
    const auto density =
        static_cast<float>(meta.getProperty<double>("density"));

    return new pbrt::FluorescentScatteringVolumeDensity(
        siga, sigs, g, Le, pbrt::BBox(p0, p1), *otw, fex, fem, mweight, epsilon,
        c, yield, gf, sscale, fscale, density);
}

pbrt::VolumeRegion* FluorescentScatteringGridVolumeFactory(
    pbrt::Transform* otw, const PropertyMap& meta)
{
    const auto p0Arr = meta.getProperty<Vector3d>("p0", {0., 0., 0.});
    const auto p1Arr = meta.getProperty<Vector3d>("p1", {1., 1., 1.});
    const pbrt::Point p0(static_cast<float>(p0Arr[0]),
    ...
    return new pbrt::FluorescentScatteringGridDensity(siga, sigs, g, Le,
                                                      pbrt::BBox(p0, p1), *otw,
                                                      nx, ny, nz, data, fex,
                                                      fem, epsilon, c, yield,
                                                      gf);
...
std::unordered_map<std::string, PBRTModel::VolumeFactory>
    PBRTModel::_volumeFactories = {
        ...
        {"fluorescence_scattering_grid",
         FluorescentScatteringGridVolumeFactory}};
...
std::vector<pbrt::Reference<pbrt::Primitive>> PBRTModel::commitToPBRT(
    const Transformation& transform, std::vector<pbrt::Sensor*>& sensorBuffer,
    std::vector<pbrt::VolumeRegion*>& volumeBuffer)
{
    ...
    sensorBuffer.insert(sensorBuffer.end(), sensorList.begin(),
                        sensorList.end());

    auto volumeList = _createVolumes(&_objectToWorld);
    volumeBuffer.insert(volumeBuffer.end(), volumeList.begin(),
                        volumeList.end());
...
SharedDataVolumePtr PBRTModel::createSharedDataVolume(
...
std::vector<pbrt::VolumeRegion*> PBRTModel::_createVolumes(pbrt::Transform* otw)
{
    ...
    for (const auto& metaObjectList : getMetaObjects())
    {
        for (const auto& metaObject : metaObjectList.second)
        {
            ...
            const auto volType =
                metaObject.getProperty<std::string>("volume_type");
...
std::vector<pbrt::Reference<pbrt::Primitive>> PBRTModel::_createSpheres(
    pbrt::Transform* otw, pbrt::Transform* wto)
{
    Primitives result;
    ...
    for (const auto& sphereList : getSpheres())
    {
        ...
        for (const auto& sphere : sphereList.second)
        {
            ...
            const auto otwS =
                pbrt::Translate(pbrt::Vector(sphere.center.x, sphere.center.y,
                                             sphere.center.z));

            std::unique_ptr<pbrt::Transform> otwFinal(new pbrt::Transform(
                pbrt::Matrix4x4::Mul(otw->GetMatrix(), otwS.GetMatrix())));
            std::unique_ptr<pbrt::Transform> wtoFinal(new pbrt::Transform(
                pbrt::Matrix4x4::Mul(wto->GetMatrix(),
            ...
            auto sphereShape = pbrt::CreateSphereShape(otwFinalPtr, wtoFinalPtr,
                                                       false, params);

            result.push_back(pbrt::Reference<pbrt::Primitive>(
                new pbrt::GeometricPrimitive(sphereShape,
                                             impl.getPBRTMaterial(), dummyAL)));
        }
    }
...
std::vector<pbrt::Reference<pbrt::Primitive>> PBRTModel::_createCylinders(
    pbrt::Transform* otw, pbrt::Transform* wto)
{
    Primitives result;
    ...
    for (const auto& cylinderList : getCylinders())
    {
        ...
        for (const auto& cylinder : cylinderList.second)
        {
            ...
            const auto direction =
                glm::normalize(cylinder.up - cylinder.center);
            ...
            auto pbrtMat =
                pbrt::LookAt(pbrt::Point(cylinder.center.x, cylinder.center.y,
                                         cylinder.center.z),
                             pbrt::Point(cylinder.up.x, cylinder.up.y,
                                         cylinder.up.z),
            ...
            std::unique_ptr<pbrt::Transform> wtoFinal(new pbrt::Transform(
                pbrt::Matrix4x4::Mul(otw->GetMatrix(), cylTrans.GetMatrix())));
            std::unique_ptr<pbrt::Transform> otwFinal(new pbrt::Transform(
                pbrt::Matrix4x4::Mul(wto->GetMatrix(),
            ...
            auto cylinderShape =
                pbrt::CreateCylinderShape(otwFinalPtr, wtoFinalPtr, false,
                                          params);
...
std::vector<pbrt::Reference<pbrt::Primitive>> PBRTModel::_createCones(
    pbrt::Transform* otw, pbrt::Transform* wto)
{
    Primitives result;
    ...
    for (const auto& coneList : getCones())
    {
        ...
        for (const auto& cone : coneList.second)
        {
            ...
            auto rUp = glm::normalize(glm::cross(direction, theoreticalUp));
            auto pbrtMat =
                pbrt::LookAt(pbrt::Point(cone.center.x, cone.center.y,
                                         cone.center.z),
            ...
            // Multiply by model transformation
            std::unique_ptr<pbrt::Transform> wtoFinal(new pbrt::Transform(
                pbrt::Matrix4x4::Mul(otw->GetMatrix(), conTrans.GetMatrix())));
            std::unique_ptr<pbrt::Transform> otwFinal(new pbrt::Transform(
                pbrt::Matrix4x4::Mul(wto->GetMatrix(),
            ...
            auto coneShape =
                pbrt::CreateConeShape(otwFinalPtr, wtoFinalPtr, false, params);

            result.push_back(pbrt::Reference<pbrt::Primitive>(
                new pbrt::GeometricPrimitive(coneShape, impl.getPBRTMaterial(),
                                             dummyAL)));
        }
    }
...
std::vector<pbrt::Reference<pbrt::Primitive>> PBRTModel::_createMeshes(
    pbrt::Transform* otw, pbrt::Transform* wto)
{
    Primitives result;
    ...
    for (const auto& meshList : getTriangleMeshes())
    {
        ...
        params.AddInt("indices", indices.data(),
                      static_cast<int>(indices.size()));

        // Verties
        std::vector<pbrt::Point> vertices(mesh.vertices.size());
        for (size_t i = 0; i < mesh.vertices.size(); ++i)
            vertices[i] = pbrt::Point(mesh.vertices[i].x, mesh.vertices[i].y,
                                      mesh.vertices[i].z);
        params.AddPoint("P", vertices.data(),
            ...
            mesh.normals.size() == mesh.vertices.size())
        {
            ...
            for (size_t i = 0; i < mesh.normals.size(); ++i)
                normals[i] = pbrt::Normal(mesh.normals[i].x, mesh.normals[i].y,
                                          mesh.normals[i].z);
            params.AddNormal("N", normals.data(),
                             static_cast<int>(mesh.normals.size()));
        }

        pbrt::Reference<pbrt::Shape> refShape(
            pbrt::CreateTriangleMeshShape(otw, wto, false, params));

        result.push_back(pbrt::Reference<pbrt::Primitive>(
            new pbrt::GeometricPrimitive(refShape, impl->getPBRTMaterial(),
                                         dummyAL)));
    }
...
std::vector<pbrt::Reference<pbrt::Primitive>> PBRTModel::_createSDFGeometries(
    pbrt::Transform* otw, pbrt::Transform* wto)
{
    Primitives result;
    ...
    for (const auto& geometryList : getSDFGeometryData().geometryIndices)
    {
        ...
        for (const auto gemetryIndex : geometryList.second)
        {
            const SDFGeometry& geom =
                getSDFGeometryData().geometries[gemetryIndex];

            pbrt::Reference<pbrt::Shape> shape(nullptr);
            switch (geom.type)
            {
            case SDFType::Sphere:
                shape = pbrt::Reference<pbrt::Shape>(
                    new PBRTSDFGeometryShape<SDFType::Sphere>(
                        otw, wto, false, &geom, &getSDFGeometryData()));
                break;
            case SDFType::Pill:
                shape = pbrt::Reference<pbrt::Shape>(
                    new PBRTSDFGeometryShape<SDFType::Pill>(
                        otw, wto, false, &geom, &getSDFGeometryData()));
                break;
            case SDFType::ConePill:
                shape = pbrt::Reference<pbrt::Shape>(
                    new PBRTSDFGeometryShape<SDFType::ConePill>(
                        otw, wto, false, &geom, &getSDFGeometryData()));
                break;
            case SDFType::ConePillSigmoid:
                shape = pbrt::Reference<pbrt::Shape>(
                    new PBRTSDFGeometryShape<SDFType::ConePillSigmoid>(
                        otw, wto, false, &geom, &getSDFGeometryData()));
                break;
            }

            result.push_back(pbrt::Reference<pbrt::Primitive>(
                new pbrt::GeometricPrimitive(shape, impl.getPBRTMaterial(),
                                             dummyAL)));
        }
    }
...
std::vector<pbrt::Sensor*> PBRTModel::_createSensors(pbrt::Transform* otw,
                                                     pbrt::Transform* wto)
{
    ...
    for (const auto& metaObjList : _geometries->_metaObjects)
    {
        for (const auto& sensorProps : metaObjList.second)
        {
            ...
            auto shapeStr =
                sensorProps.getProperty<std::string>("sensor_shape_type");
            if (shapeStr != std::string("rectangle") &&
                shapeStr != std::string("disk"))
            {
            ...
            const auto sensorRotation =
                sensorProps.getProperty<std::array<double, 4>>(
                    "sensor_rotation", {0.0, 1.0, 0.0, 0.0});
            ...
            const auto localTransMatrix = pbrt::Matrix4x4::Mul(
                pbrt::Matrix4x4::Mul(localRot.GetMatrix(),
            ...
            // Final transforms
            std::unique_ptr<pbrt::Transform> otwFinal(new pbrt::Transform(
                pbrt::Matrix4x4::Mul(otw->GetMatrix(),
            ...
            const auto reference =
                sensorProps.getProperty<std::string>("reference", "sensor");
            const auto xPixels = sensorProps.getProperty<int>("xpixels", 0);
            const auto yPixels = sensorProps.getProperty<int>("ypixels", 0);
            const auto fov =
                static_cast<float>(sensorProps.getProperty<double>("fov", 45.));

            if (shapeStr == std::string("rectangle"))
            {
                const auto height = static_cast<float>(
                    sensorProps.getProperty<double>("sensor_shape_height",
                                                    0.0));
                const auto x = static_cast<float>(
                    sensorProps.getProperty<double>("sensor_shape_x", 1.0));
                const auto y = static_cast<float>(
                    sensorProps.getProperty<double>("sensor_shape_y", 1.0));
                pbrt::Shape* shape =
                    new pbrt::Rectangle(otwFinalPtr, wtoFinalPtr, false, height,
                                        x, y);
                sensor = new pbrt::Sensor(shapeStr, reference, shape, xPixels,
                                          yPixels, x, y, fov);
            }
            else // disk otherwise
            {
                ...
                const auto radius = static_cast<float>(
                    sensorProps.getProperty<double>("sensor_shape_radius",
                                                    1.0));
                const auto innerrad = static_cast<float>(
                    sensorProps.getProperty<double>("sensor_shape_innerradius",
                                                    0.0));
                const auto phi = static_cast<float>(
                    sensorProps.getProperty<double>("sensor_shape_phimax",
                                                    360.0));

                pbrt::Shape* shape =
                    new pbrt::Disk(otwFinalPtr, wtoFinalPtr, false, height,
                                   radius, innerrad, phi);

                sensor = new pbrt::Sensor(shapeStr, reference, shape, xPixels,
                                          yPixels, 2 * radius, 2 * radius, fov);
            }
