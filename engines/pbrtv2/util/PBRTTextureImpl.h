...
{
public:
    PBRTTextureImpl(pbrt::TextureMapping2D* mapping,
                    const Texture2D& srcTexture,
                    pbrt::ImageWrap warpMethod = pbrt::ImageWrap::TEXTURE_REPEAT,
                    const bool doTrilinearFilter = true, const float maxAnisotropicFilter = 8.f,
                    const float scale = 1.f, const float gamma = 1.f);
    ~PBRTTextureImpl()
    {
        delete _mapping;
        delete _mipMap;
 ...
 : _mapping(std::move(mapping))
{
    ...
    for(uint32_t i = 0; i < size; ++i)
    {
        ...
        float srcVals[] = {data[idx] * div,
                           data[idx+1] * div,
                           data[idx+2] * div};
        // scale + gamma correct
        pbrt::RGBSpectrum& result = converted[i];
        const float rbgVals[3] = {
            (gamma != 1.f? powf(scale * srcVals[0], gamma) : scale * srcVals[0]),
            (gamma != 1.f? powf(scale * srcVals[1], gamma) : scale * srcVals[1]),
            (gamma != 1.f? powf(scale * srcVals[2], gamma) : scale * srcVals[2])
        };
        result = pbrt::RGBSpectrum::FromRGB(rbgVals);
    }
    // Create mip mapping
    _mipMap = new pbrt::MIPMap<pbrt::RGBSpectrum>(static_cast<int>(resx), static_cast<int>(resy),
 ...
 : _mapping(std::move(mapping))
{
    ...
    // Create mip mapping
    _mipMap = new pbrt::MIPMap<pbrt::SampledSpectrum>(static_cast<int>(resx), static_cast<int>(resy),
                                                  converted.data(), doTrilinearFilter,
                                                  maxAnisotropicFilter, warpMethod);
}

template<>
PBRTTextureImpl<float>::PBRTTextureImpl(
                pbrt::TextureMapping2D* mapping,
                const Texture2D& srcTexture, pbrt::ImageWrap warpMethod,
                bool doTrilinearFilter, const float maxAnisotropicFilter,
                const float scale, const float gamma)
 : _mapping(std::move(mapping))
{
    ...
    for(uint32_t i = 0; i < size; ++i)
    {
        ...
        // scale + gamma correct
        converted[i] = gamma != 1.f? powf(scale * srcVal, gamma) : scale * srcVal;

    }
    // Create mip mapping
    //pbrt::Point2i pbrtRes (static_cast<int>(resx), static_cast<int>(resy));
    _mipMap = new pbrt::MIPMap<float>(static_cast<int>(resx), static_cast<int>(resy),
                                      converted.data(), doTrilinearFilter, maxAnisotropicFilter,
                                      warpMethod);
}
